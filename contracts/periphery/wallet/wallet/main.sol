...
contract FluidWalletErrorsAndEvents {
    ...
    event Executed(address indexed owner, uint256 indexed tokenId);
...
contract FluidWalletImplementation is FluidWalletVariables, FluidWalletErrorsAndEvents {
    ...
    /// @dev transfers the position NFT held by this wallet back to `owner_`.
    function _returnNft(address owner_, uint256 tokenId_) internal virtual {
    ...
    function executeOperation(
        ...
        if (
            !(_transientAllowHash == bytes32(keccak256(abi.encode(data_, block.timestamp))) &&
    ...
    function _sweepTokens(address owner_, uint256 tokenId_) internal {
        uint256 tokenConfig_ = IFluidVaultFactory(VAULT_FACTORY).readFromStorage(
            _calculateStorageSlotUintMapping(3, tokenId_)
        );
