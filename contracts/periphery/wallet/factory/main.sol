...
contract FluidWalletFactoryVariables is Initializable, OwnableUpgradeable {
    ...
    IFluidVaultFactory public immutable VAULT_T1_FACTORY;

    address public immutable WALLET_PROXY;
...
contract FluidWalletFactory is FluidWalletFactoryVariables, FluidWalletFactoryErrorsAndEvents, UUPSUpgradeable {
    ...
    function onERC721Received(
        ...
        _forwardNft(params_.wallet, params_.nftId, params_.owner, params_.actions);
    ...
    /// @dev forwards the received NFT to the user's wallet clone, passing owner + actions to the wallet's
    ///      `onERC721Received` (overridable, e.g. permissioned variant uses the gated safe transfer).
    function _forwardNft(address wallet_, uint256 nftId_, address owner_, Action[] memory actions_) internal virtual {
        VAULT_T1_FACTORY.safeTransferFrom(address(this), wallet_, nftId_, abi.encode(owner_, actions_));
    }
