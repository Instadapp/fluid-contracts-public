...
contract VaultT1Migrator is Owned {
    ...
    function setFlashloanConfig(address token_, uint256 route_, uint256 amount_) public onlyOwner {
        flashloanConfig[token_] = FlashloanConfig({ amount: amount_, route: route_ });
    ...
    function executeOperation(
        ...
        uint256 value_;
        {
            ...
            (, params_.withdrawAmount, params_.paybackAmount) = IFluidVaultT1(params_.vaultFrom).operate{
                value: value_
            }(params_.nftId, type(int256).min, type(int256).min, address(this));
...

        {
            ...
            (uint256 nftId_, , ) = IFluidVaultT1(params_.vaultTo).operate{ value: value_ }(
                0,
