...
contract VaultLiquidatorImplementationV1 {
    ...
    /// @dev Debt-token spender before liquidate. Public: the vault. Permissioned fork: thin class-4 wrapper.
    function _liquidationSpender(address vault_, uint256 vaultType_) internal view virtual returns (address) {
        return vault_;
    }

    /// @dev T1 liquidate. Public: vault directly. Permissioned fork: thin wrapper (collateral to address(this)).
    function _liquidateT1(
        address vault_,
        uint256 debtAmt_,
        uint256 colPerUnitDebt_,
        bool absorb_,
        uint256 value_
    ) internal virtual returns (uint256 debtAmount_, uint256 collateralAmount_) {
        return IFluidVaultT1(vault_).liquidate{ value: value_ }(debtAmt_, colPerUnitDebt_, address(this), absorb_);
    }

    function _validateParams(LiquidationParams memory params_) internal view {
        if (params_.expiration > 0 && params_.expiration < block.timestamp)
            revert FluidVaultLiquidator__InvalidTimestamp();

        uint256 vaultVariables_ = IFluidVaultT1(params_.vault).readFromStorage(0);

        int256 topTick_ = _tickHelper(((vaultVariables_ >> 2) & X20));

        if (params_.topTick > topTick_ && params_.topTick != type(int256).min)
            revert FluidVaultLiquidator__InvalidTopTick();
    }
    ...
    function liquidateDust(LiquidationDustParams memory params_) public _onlyDelegateCall {
        ...
        address spender_ = _liquidationSpender(params_.vault, params_.vaultType);
        uint256 value_;
        if (params_.debtToken != ETH_ADDRESS) {
            SafeApprove.safeApprove(params_.debtToken, spender_, 0);
            SafeApprove.safeApprove(params_.debtToken, spender_, params_.debtAmount);
            value_ = 0;
        } else {
        ...
        if (params_.vaultType == 1) {
            (debtAmount_, collateralAmount_) = _liquidateT1(
                params_.vault,
                params_.token0DebtAmt,
                params_.colPerUnitDebt,
                params_.absorb,
                value_
            );
        } else if (params_.vaultType == 2) {
            (debtAmount_, collateralAmount_, , ) = IFluidVaultT2(params_.vault).liquidate{ value: value_ }(
                params_.token0DebtAmt,
                params_.colPerUnitDebt,
                params_.token0ColAmtPerUnitShares,
                params_.token1ColAmtPerUnitShares,
                address(this),
                params_.absorb
            );
        } else if (params_.vaultType == 3) {
            (debtAmount_, collateralAmount_) = IFluidVaultT3(params_.vault).liquidate{ value: value_ }(
                params_.token0DebtAmt,
                params_.token1DebtAmt,
                params_.debtSharesMin,
                params_.colPerUnitDebt,
                address(this),
                params_.absorb
            );
        } else if (params_.vaultType == 4) {
            (debtAmount_, collateralAmount_, , ) = IFluidVaultT4(params_.vault).liquidate{ value: value_ }(
                params_.token0DebtAmt,
                params_.token1DebtAmt,
                params_.debtSharesMin,
                params_.colPerUnitDebt,
    ...
    function executeOperation(
...

        {
            ...
            if (params_.flashloanToken != ETH_ADDRESS) {
                SafeApprove.safeApprove(params_.flashloanToken, spender_, 0);
                SafeApprove.safeApprove(params_.flashloanToken, spender_, params_.flashloanAmount);
                value_ = 0;
            } else {
