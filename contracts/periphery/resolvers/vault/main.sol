...
import { IFluidStorageReadable, IFluidAccessTypeView, IFluidDeploymentNameView } from "../../../libraries/utils/minimalInterfaces.sol";
...
contract FluidVaultResolver is Helpers, ResolverHelpers {
    ...
    function getDeploymentName() external view returns (string memory deploymentName_) {
        try IFluidDeploymentNameView(address(FACTORY)).deploymentName() returns (string memory name_) {
    ...
    function _getVaultConfig(address vault_, uint vaultType_) internal view returns (Configs memory configs_) {
        ...
        if (configs_.oracle != address(0)) {
            (configs_.oraclePriceOperate, configs_.oraclePriceLiquidate) = _fetchOraclePrices(configs_.oracle);
        }
        ...
        )
    {
        ...
        if (userSupplyLiquidityOrDexData_ > 0) {
            ...
            if (constantsVariables_.supplyToken.token1 == address(0)) {
                ...
                balanceOf_ += _getLiquidityExternalBalances(
                    constantsVariables_.supplyToken.token0,
                    address(constantsVariables_.liquidity)
                );
        ...
        if (userBorrowLiquidityOrDexData_ > 0) {
            if (constantsVariables_.borrowToken.token1 == address(0)) {
                ...
                balanceOf_ += _getLiquidityExternalBalances(
                    constantsVariables_.borrowToken.token0,
                    address(constantsVariables_.liquidity)
                );
    ...
    function getVaultEntireData(address vault_) public view returns (VaultEntireData memory vaultData_) {
        ...
        if (vaultType_ != 0) {
            ...
            vaultData_.accessType = _getAccessType(vault_);
        }
    }
