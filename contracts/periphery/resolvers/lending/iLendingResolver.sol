...
interface IFluidLendingResolver {
    ...
    /// @notice returns the factory deployment name when present (permissioned stack); empty string on production factories.
    function getDeploymentName() external view returns (string memory);
    ...
    /// fToken type, name, symbol, decimals, underlying asset, total amounts, convertTo values, and APR fields.
    ...
    /// @dev All resolver APR fields use Liquidity scale (`100` = 1%, divide by 100 for % display).
    ...
    /// @dev All APR fields use Liquidity resolver scale (`100` = 1%). Streaming `totalRate_` is
    /// `liquidityRate_ + relativeModelRate_` (additive per fToken exchange-price math, not multiplicative).
    /// Static `staticModelRate_` is the signed Liquidity offset; `totalRate_ = max(0, liquidityRate_ + staticModelRate_)`.
    /// `rewardsActive_` is true while the wired program is currently accruing (false after stop/natural end
    /// even if the model remains wired — e.g. `isStaticRate_` may still be true).
    ...
    /// @dev Tuple shape is shared for streaming and static models. All fields are raw on-chain values
    ///      (`1e12` = 1% for rate fields), unlike APR fields on `getFTokenDetails` (`100` = 1%).
    ///      `maxRateOrStaticRate_`: streaming = model max-rate ceiling (`>= 0`); static = signed Liquidity
    ///      offset from `getStaticConfig().staticRate` (may be negative). Static `|offset|` ceiling remains
    ///      `getStaticConfig().maxRate`. Static: `startTvl_` and `rewardAmount_` are `0`.
