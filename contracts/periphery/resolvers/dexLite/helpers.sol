...
abstract contract Helpers is ImmutableVariables {
    ...
    function _calculateDexId(DexKey memory dexKey_) internal pure returns (bytes32) {
        return keccak256(abi.encode(dexKey_));
    }

    function _calculatePoolStateSlot(bytes32 dexId, uint256 baseSlot) internal pure returns (bytes32) {
        return keccak256(abi.encode(dexId, baseSlot));
    }
    ...
    function _getPricesAndReserves(
        ...
        if (((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_CENTER_PRICE_SHIFT_ACTIVE) & X1) == 0) {
            ...
            if (prices_.centerPrice == 0) {
                ...
                prices_.centerPrice = ICenterPrice(AC.addressCalc(DEPLOYER_CONTRACT, prices_.centerPrice)).centerPrice(
                    dexKey_.token0,
                    dexKey_.token1
                );
            }
        ...
        unchecked {
            if (prices_.upperRangePrice < 1e38) {
                ...
                temp_ = FPM.sqrt((prices_.upperRangePrice / 1e18) * (prices_.lowerRangePrice / 1e18)) * 1e18;
            }
        }
