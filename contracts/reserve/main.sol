...
contract FluidReserveContract is Error, ReserveContractAuth, UUPSUpgradeable {
    ...
    modifier onlyGovernance() {
        if (GOVERNANCE != msg.sender) {
            revert FluidReserveContractError(ErrorTypes.ReserveContract__OnlyGovernanceAllowed);
        }
        _;
    }
    ...
    function rebalanceVault(address protocol_, uint256 value_) public payable onlyRebalancer {
        ...
        if (
            value_ > 0 &&
            !(constants_.supplyToken == NATIVE_TOKEN_ADDRESS || constants_.borrowToken == NATIVE_TOKEN_ADDRESS)
        ) {
            revert FluidReserveContractError(ErrorTypes.ReserveContract__WrongValueSent);
        }
    ...
    function withdrawFunds(
        ...
        for (uint256 i = 0; i < tokens_.length; i++) {
            if (tokens_[i] == NATIVE_TOKEN_ADDRESS) {
                SafeTransfer.safeTransferNative(receiver, amounts_[i]);
            } else {
                SafeTransfer.safeTransfer(address(tokens_[i]), receiver, amounts_[i]);
            }
            emit LogWithdrawFunds(tokens_[i], amounts_[i], receiver);
        }
    }
