...
contract FluidReserveContract is Error, ReserveContractAuth, UUPSUpgradeable {
    ...
    /// @notice Checks that the sender is an allowed rebalancer
    ...
    function withdrawFunds(
        ...
        if (
            receiver_ == address(0) || // handle case where treasury and buyback contract addresses dont exist on a chain
            ((msg.sender == TEAM_MULTISIG) &&
                (receiver_ != TREASURY_ADDRESS) &&
                (receiver_ != BUYBACK_CONTRACT_ADDRESS))
        ) {
            revert FluidReserveContractError(ErrorTypes.ReserveContract__Unauthorized);
        }
