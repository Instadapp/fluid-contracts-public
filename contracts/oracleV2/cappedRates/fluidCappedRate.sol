...
import { FluidCenterPrice } from "../centerPrices/fluidCenterPrice.sol";
...
abstract contract CappedRateInternals is Variables, Events, OracleError {
    ...
    function _percentDiffForValue(
        ...
        if (oldValue_ == 0) {
            return newValue_ > 0 ? _SIX_DECIMALS : 0;
        }
        unchecked {
    ...
    function _updateRates(bool forceUpdate_) internal virtual returns (uint256 newRate_) {
        ...
        if (
            ...
            if (forceUpdate_) {
                ...
                allowedMaxYieldJumps_ = uint256((slot0_.flags & _FLAG_BITMASK_ALLOW_MAX_YIELD_JUMPS) >> 5) + 1;
                if (allowedMaxYieldJumps_ > _X3) {
                    allowedMaxYieldJumps_ = _X3; // force max value possible 7
                }
            } else {
    ...
    function _calcMaxAPRCappedRate(
        ...
        unchecked {
            ...
            uint256 allowedMaxYieldJumps_ = (slot0_.flags & _FLAG_BITMASK_ALLOW_MAX_YIELD_JUMPS) >> 5;
...
abstract contract CappedRateAdmin is Variables, Events, OracleError {
    ...
    /// @notice Updates the maxAPRPercent_ config, in 1e4 percent (1% = 1e4). Only callable by Liquidity Layer guardians.
...
abstract contract FluidCappedRateBase is CappedRateInternals, CappedRateAdmin, IFluidCappedRate {
    ...
    /// @inheritdoc IFluidOracle
    function getExchangeRateOperateRaw() public view virtual returns (uint256 exchangeRate_) {
        return getExchangeRateOperate();
    }

    /// @inheritdoc IFluidOracle
    function getExchangeRateLiquidateRaw() public view virtual returns (uint256 exchangeRate_) {
        return getExchangeRateLiquidate();
    }

    /// @inheritdoc IFluidOracle
    function getExchangeRateRaw() public view virtual returns (uint256 exchangeRate_) {
