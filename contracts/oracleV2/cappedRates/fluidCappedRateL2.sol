...
import { FluidCenterPriceL2 } from "../centerPrices/fluidCenterPriceL2.sol";
...
abstract contract FluidCappedRateL2 is FluidCappedRateBase, FluidCenterPriceL2 {
    ...
    /// @inheritdoc IFluidOracleWithDebt
    ...
    /// @inheritdoc IFluidOracle
    /// @dev L2 "Raw" = skips only `_ensureSequencerUpAndValid()`. The rate is still the capped operate rate
    ///      (delegates to the base `getExchangeRateOperate()`); caps are NOT removed.
    function getExchangeRateOperateRaw() public view virtual override returns (uint256 exchangeRate_) {
        return FluidCappedRateBase.getExchangeRateOperate();
    }

    /// @inheritdoc IFluidOracle
    /// @dev L2 "Raw" = skips only `_ensureSequencerUpAndValid()`. The rate is still the capped liquidate rate
    ///      (delegates to the base `getExchangeRateLiquidate()`); caps are NOT removed.
    function getExchangeRateLiquidateRaw() public view virtual override returns (uint256 exchangeRate_) {
        return FluidCappedRateBase.getExchangeRateLiquidate();
    }

    /// @inheritdoc IFluidOracle
    /// @dev L2: skips `_ensureSequencerUpAndValid()`; delegates to the uncapped base rate logic (`getExchangeRate()`).
    function getExchangeRateRaw() public view virtual override returns (uint256 exchangeRate_) {
        return FluidCappedRateBase.getExchangeRate();
    }
}
