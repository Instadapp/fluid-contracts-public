...
import { CenterPriceError as OracleError } from "../error.sol";
...
contract FluidCappedRateInvertCenterPrice is FluidCenterPrice {
    ...
    constructor(string memory infoName_, address fluidCappedRate_) FluidCenterPrice(infoName_) {
        if (fluidCappedRate_ == address(0)) {
            revert FluidCenterPriceError(ErrorTypes.CenterPrice__InvalidParams);
        }
    ...
    function centerPrice() external override returns (uint256 price_) {
        uint256 rate_ = FluidCenterPrice(FLUID_CAPPED_RATE).centerPrice();
        if (rate_ == 0) {
            revert FluidCenterPriceError(ErrorTypes.CenterPrice__RateZero);
        }
        return 1e54 / rate_;
    }
}
