...
contract FluidDexT1DeploymentLogic is Error {
    /// @dev SSTORE2 pointers for PoolT1 creation code - stored in two parts for concat
    address internal immutable POOL_T1_CREATIONCODE_ADDRESS_1;
    address internal immutable POOL_T1_CREATIONCODE_ADDRESS_2;

    /// @dev SSTORE2 pointers for col operations creation code
    ...
    /// @dev SSTORE2 pointers for debt operations creation code
    ...
    constructor(
        ...
        COL_OPERATIONS_CREATIONCODE_ADDRESS_1 = colOperations1_;
        COL_OPERATIONS_CREATIONCODE_ADDRESS_2 = colOperations2_;
        DEBT_OPERATIONS_CREATIONCODE_ADDRESS_1 = debtOperations1_;
        DEBT_OPERATIONS_CREATIONCODE_ADDRESS_2 = debtOperations2_;
    }
    ...
    function dexT1(
        ...
        constants_.implementations.perfectOperationsAndSwapOut = address(0); // does not exist anymore
    ...
    function colOperationsCreationCode() public view returns (bytes memory) {
        return
            BytesSliceAndConcat.bytesConcat(
                SSTORE2.read(COL_OPERATIONS_CREATIONCODE_ADDRESS_1),
                SSTORE2.read(COL_OPERATIONS_CREATIONCODE_ADDRESS_2)
            );
    }
    ...
    function debtOperationsCreationCode() public view returns (bytes memory) {
        return
            BytesSliceAndConcat.bytesConcat(
                SSTORE2.read(DEBT_OPERATIONS_CREATIONCODE_ADDRESS_1),
                SSTORE2.read(DEBT_OPERATIONS_CREATIONCODE_ADDRESS_2)
            );
    }
    ...
    function perfectOperationsCreationCode() public pure returns (bytes memory) {
        return new bytes(0); // does not exist anymore
    }
