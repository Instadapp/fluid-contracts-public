...
interface IShifting {
    ...
    function _calcCenterPrice(uint dexVariables_, uint dexVariables2_) external payable returns (uint);
}
...
abstract contract CoreHelpers is Variables, ImmutableVariables, Events {
    ...
    function _utilizationVerify(uint utilizationLimit_, bytes32 exchangePriceSlot_) internal view {
        if (utilizationLimit_ < THREE_DECIMALS) {
            unchecked {
                ...
                if (
                    ((LIQUIDITY.readFromStorage(exchangePriceSlot_) >>
                        LiquiditySlotsLink.BITS_EXCHANGE_PRICES_UTILIZATION) & X14) > utilizationLimit_ * 10
    ...
    function _verifyToken1Reserves(
        ...
        unchecked {
            minLiquidity_ = 1e27 * minLiquidity_; // safe unchecked, min liquidity is a non-zero constant value passed in
        }
        if (((token1Reserves_) < ((token0Reserves_ * centerPrice_) / minLiquidity_))) {
            revert FluidDexError(ErrorTypes.DexT1__TokenReservesTooLow);
        }
    }
    ...
    function _calculateReservesOutsideRange(
        ...
        if (p3_ < 1e50) {
            unchecked {
                p3_ = (p3_ * 1e27) / p1_; // multiplication can not overflow, p1_ can not be 0 as p2_ calculation above would revert
            }
        } else {
            unchecked {
                p3_ = p3_ / p1_; // p1_ can not be 0 as p2_ calculation above would revert
            }
            p3_ = p3_ * 1e27;
        }
    ...
    function _calculateDebtReserves(
        ...
        // final x equals:
        // x = (part1 + (part2 + part1^2)^(1/2))
        int p1_ = (int(dx_ * gp_) - int(dy_ * 1e27));
        unchecked {
            p1_ = p1_ / (2 * 1e27);
        }
        uint p2_ = (dx_ * dy_);
        ...
        p2_ = ry_ * ry_; // reusing unused var p2_
        if (ry_ < 1e25) {
            unchecked {
                // iry_ can not be 0 because of above < SIX_DECIMALS check
                // p2_ * 1e27 can not overflow because ry_ is <1e25
                iry_ = (p2_ * 1e27) / iry_;
            }
        } else {
    ...
    function _updateOracle(uint newPrice_, uint centerPrice_, uint dexVariables_) internal view returns (uint) {
        // time difference between last & current swap
        uint temp_ = block.timestamp - ((dexVariables_ >> 121) & X33);
