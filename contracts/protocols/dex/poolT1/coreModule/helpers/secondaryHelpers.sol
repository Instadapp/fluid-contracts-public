...
abstract contract SecondaryHelpers is CoreHelpers {
    ...
    function _getSwapAndDeposit(uint c_, uint d_, uint e_, uint f_, uint i_) internal pure returns (uint shares_) {
        ...
        uint temp2_ = 4 * c_ * e_;
        uint amtToSwap_ = (FixedPointMathLib.sqrt((temp2_ + (temp_ * temp_))) - temp_);
        unchecked {
            amtToSwap_ = amtToSwap_ / 2;
        }
        ...
        temp_ = c_ * (SIX_DECIMALS - 1);
        unchecked {
            if ((amtToSwap_ > (temp_ / SIX_DECIMALS)) || (amtToSwap_ < (c_ / SIX_DECIMALS)))
    ...
    function _getWithdrawAndSwap(uint c_, uint d_, uint e_, uint f_, uint g_) internal pure returns (uint shares_) {
        ...
        // This prevents extreme scenarios and maybe potential precision issues
        temp_ = g_ * (SIX_DECIMALS - 1);
        unchecked {
            if (tokenAxa_ > (temp_ / SIX_DECIMALS) || tokenAxa_ < (g_ / SIX_DECIMALS))
    ...
    function _getSwapAndPaybackOneTokenPerfectShares(
        ...
        l_ = c_ * (SIX_DECIMALS - 1); // reusing l_, not needed var hereafter
        unchecked {
            if ((tokenAmt_ > l_ / SIX_DECIMALS)) revert FluidDexError(ErrorTypes.DexT1__SwapAndPaybackTooLowOrTooHigh);
        }
    }
    ...
    function _getSwapAndPayback(uint c_, uint d_, uint e_, uint f_, uint g_) internal pure returns (uint shares_) {
        ...
        temp_ = g_ * (SIX_DECIMALS - 1);
        unchecked {
            if ((amtToSwap_ > temp_ / SIX_DECIMALS) || (amtToSwap_ < (g_ / SIX_DECIMALS)))
    ...
    modifier _onlyDelegateCall() {
        _enforceDelegateCall(); // use internal method to reduce bytecode
        _;
    }

    /// @dev Checks that the current call is via delegatecall and not a direct call.
    ///      Used as a helper for `_onlyDelegateCall` modifier above.
    function _enforceDelegateCall() internal view {
