...
abstract contract CoreInternals is Helpers {
    function _swapIn(DexKey calldata dexKey_, bool swap0To1_, uint256 amountIn_) internal returns (uint256 amountOut_) {
        ...
        uint256 token0AdjustedSupply_ = (dexVariables_ >>
            DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_0_TOTAL_SUPPLY_ADJUSTED) & X60;
        uint256 token1AdjustedSupply_ = (dexVariables_ >>
            DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_1_TOTAL_SUPPLY_ADJUSTED) & X60;
        (
        ...
        unchecked {
            if (swap0To1_) {
                ...
                uint256 fee_ = (amountIn_ * ((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_FEE) & X13)) /
                    SIX_DECIMALS;
                // amountOut = (amountIn * iReserveOut) / (iReserveIn + amountIn)
                amountOut_ =
                    ((amountIn_ - fee_) * token1ImaginaryReserves_) /
                    (token0ImaginaryReserves_ + (amountIn_ - fee_));
                token0AdjustedSupply_ +=
                    amountIn_ -
                    ((fee_ * ((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_REVENUE_CUT) & X7)) / TWO_DECIMALS);
                ...
                if (
                    ((token1AdjustedSupply_) <
                        ((token0AdjustedSupply_ * centerPrice_) / (PRICE_PRECISION * MINIMUM_LIQUIDITY_SWAP)))
                ) {
                ...
                // amountOut = (amountIn * iReserveOut) / (iReserveIn + amountIn)
                amountOut_ =
                    ((amountIn_ - fee_) * token0ImaginaryReserves_) /
                    (token1ImaginaryReserves_ + (amountIn_ - fee_));
                token1AdjustedSupply_ +=
                    amountIn_ -
                    ((fee_ * ((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_REVENUE_CUT) & X7)) / TWO_DECIMALS);
                ...
                if (
                    ((token0AdjustedSupply_) <
                        ((token1AdjustedSupply_ * PRICE_PRECISION) / (centerPrice_ * MINIMUM_LIQUIDITY_SWAP)))
                ) {
                    revert TokenReservesRatioTooHigh(token0AdjustedSupply_, token1AdjustedSupply_);
                }
            }
        ...
        bool persistCenterPrice_;
        {
            ...
            if (rebalancingStatus_ > 0) {
                ...
                unchecked {
                    price_ = (
                        swap0To1_
                            ? ((token1ImaginaryReserves_ - amountOut_) * PRICE_PRECISION) /
                                (token0ImaginaryReserves_ + amountIn_)
                            : ((token1ImaginaryReserves_ + amountIn_) * PRICE_PRECISION) /
                                (token0ImaginaryReserves_ - amountOut_)
                    );
                }
                rebalancingStatus_ = _getRebalancingStatus(
                    dexVariables_,
                    dexId_,
                    rebalancingStatus_,
                    price_,
            ...
            persistCenterPrice_ =
                rebalancingStatus_ > 1 ||
                ((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_CENTER_PRICE_SHIFT_ACTIVE) & X1) == 1;
            if (persistCenterPrice_) {
                _centerPriceShift[dexId_] =
                    (_centerPriceShift[dexId_] &
                        ~(X33 << DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_LAST_INTERACTION_TIMESTAMP)) |
        ...
        dexVariables_ =
            (_dexVariables[dexId_] & ~(X120 << DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_0_TOTAL_SUPPLY_ADJUSTED)) |
        ...
        if (persistCenterPrice_) {
            dexVariables_ =
                (dexVariables_ & ~(X40 << DSL.BITS_DEX_LITE_DEX_VARIABLES_CENTER_PRICE)) |
        ...
        if (swap0To1_) {
            emit LogSwap(
                (uint256(uint64(bytes8(dexId_))) << DSL.BITS_DEX_LITE_SWAP_DATA_DEX_ID) |
    ...
    function _swapOut(
        ...
        unchecked {
            if (swap0To1_) {
                ...
                uint256 fee_ = ((amountIn_ * SIX_DECIMALS) /
                    (SIX_DECIMALS - ((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_FEE) & X13))) - amountIn_;
                amountIn_ += fee_;
                token0AdjustedSupply_ +=
                    amountIn_ -
                    ((fee_ * ((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_REVENUE_CUT) & X7)) / TWO_DECIMALS);
