...
abstract contract Helpers is CommonImport {
        ...
        unchecked {
            if ((startTimeStamp_ + shiftDuration_) < block.timestamp) {
                ...
                _dexVariables[dexId_] =
                    _dexVariables[dexId_] &
                    ~uint256(1 << DSL.BITS_DEX_LITE_DEX_VARIABLES_RANGE_PERCENT_SHIFT_ACTIVE);
    ...
    function _calcThresholdShifting(
        ...
        unchecked {
            if ((startTimeStamp_ + shiftDuration_) < block.timestamp) {
                ...
                _dexVariables[dexId_] =
                    _dexVariables[dexId_] &
                    ~uint256(1 << DSL.BITS_DEX_LITE_DEX_VARIABLES_THRESHOLD_PERCENT_SHIFT_ACTIVE);
    ...
    function _getPricesAndReserves(
            ...
            unchecked {
                ...
                if (temp_ == 2) {
                    ...
                    if (timeElapsed_ < shiftingTime_) {
                        centerPrice_ =
                            centerPrice_ +
                            (((upperRangePrice_ - centerPrice_) * timeElapsed_) / shiftingTime_);
                    } else {
                        // 100% price shifted
                    ...
                    if (timeElapsed_ < shiftingTime_) {
                        centerPrice_ =
                            centerPrice_ -
                            (((centerPrice_ - lowerRangePrice_) * timeElapsed_) / shiftingTime_);
                    } else {
                        // 100% price shifted
        ...
        if (temp_ < 1e27) {
            (token0ImaginaryReserves_, token1ImaginaryReserves_) = _calculateReservesOutsideRange(
                temp_,
                upperRangePrice_,
                token0Supply_,
                token1Supply_
    ...
    function _getRebalancingStatus(
        ...
        unchecked {
            upperThreshold_ = (centerPrice_ +
                ((upperRange_ - centerPrice_) * (TWO_DECIMALS - upperThreshold_)) /
                TWO_DECIMALS);
            lowerThreshold_ = (centerPrice_ -
                ((centerPrice_ - lowerRange_) * (TWO_DECIMALS - lowerThreshold_)) /
                TWO_DECIMALS);
        }
        ...
        if (price_ > upperThreshold_) {
            if (rebalancingStatus_ != 2) {
                _dexVariables[dexId_] =
                    (_dexVariables[dexId_] & ~(X2 << DSL.BITS_DEX_LITE_DEX_VARIABLES_REBALANCING_STATUS)) |
    ...
    function _transferTokens(
        ...
        if (tokenIn_ == NATIVE_TOKEN) {
            if (isCallback_ && msg.value == 0) {
                ...
                if (address(this).balance - ethBalance_ < amountIn_)
                    revert InsufficientNativeTokenReceived(address(this).balance - ethBalance_, amountIn_);
            } else {
            ...
            if (isCallback_) {
                ...
                if ((IERC20(tokenIn_).balanceOf(address(this)) - tokenInBalance_) < amountIn_) {
                    revert InsufficientERC20Received(
                        IERC20(tokenIn_).balanceOf(address(this)) - tokenInBalance_,
                        amountIn_
                    );
                }
            } else {
