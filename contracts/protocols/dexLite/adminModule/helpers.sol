...
abstract contract AdminModuleHelpers is AdminModuleImmutableVariables {
    ...
    function _calculateNumeratorAndDenominatorPrecisions(
        uint256 decimals_
    ) internal pure returns (uint256 numerator_, uint256 denominator_) {
    ...
    function _getPrice(
        ...
        if (((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_CENTER_PRICE_SHIFT_ACTIVE) & X1) == 0) {
            ...
            if (centerPrice_ == 0) {
                ...
                centerPrice_ = ICenterPrice(AC.addressCalc(DEPLOYER_CONTRACT, centerPrice_)).centerPrice(
                    dexKey_.token0,
                    dexKey_.token1
                );
            }
