...
contract FluidDexLiteAdminModule is AdminModuleHelpers {
    ...
    function initialize(InitializeParams memory i_) external payable _onlyDelegateCall {
        ...
        v_.token0Decimals = i_.dexKey.token0 == NATIVE_TOKEN
            ? NATIVE_TOKEN_DECIMALS
            : IERC20WithDecimals(i_.dexKey.token0).decimals();
        v_.token1Decimals = i_.dexKey.token1 == NATIVE_TOKEN
            ? NATIVE_TOKEN_DECIMALS
            : IERC20WithDecimals(i_.dexKey.token1).decimals();
        ...
        (v_.token0NumeratorPrecision, v_.token0DenominatorPrecision) = _calculateNumeratorAndDenominatorPrecisions(
            v_.token0Decimals
        );
        (v_.token1NumeratorPrecision, v_.token1DenominatorPrecision) = _calculateNumeratorAndDenominatorPrecisions(
            v_.token1Decimals
        );
        ...
        _dexVariables[v_.dexId] =
            ...
            (BM.toBigNumber(i_.centerPrice, BIG_COEFFICIENT_SIZE, DEFAULT_EXPONENT_SIZE, BM.ROUND_DOWN) <<
                DSL.BITS_DEX_LITE_DEX_VARIABLES_CENTER_PRICE) |
            ...
            (((i_.token0Amount * v_.token0NumeratorPrecision) / v_.token0DenominatorPrecision) <<
                DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_0_TOTAL_SUPPLY_ADJUSTED) |
            (((i_.token1Amount * v_.token1NumeratorPrecision) / v_.token1DenominatorPrecision) <<
                DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_1_TOTAL_SUPPLY_ADJUSTED);
        ...
        _centerPriceShift[v_.dexId] =
            ...
            (BM.toBigNumber(i_.maxCenterPrice, SMALL_COEFFICIENT_SIZE, DEFAULT_EXPONENT_SIZE, BM.ROUND_UP) <<
                DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_MAX_CENTER_PRICE) |
            (BM.toBigNumber(i_.minCenterPrice, SMALL_COEFFICIENT_SIZE, DEFAULT_EXPONENT_SIZE, BM.ROUND_DOWN) <<
                DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_MIN_CENTER_PRICE);

        _dexesList.push(i_.dexKey);

        emit LogInitialize(i_.dexKey, bytes8(v_.dexId), _dexVariables[v_.dexId], _centerPriceShift[v_.dexId], i_);
    }
    ...
    function updateFeeAndRevenueCut(
        ...
        _dexVariables[dexId_] =
            (dexVariables_ & ~(X20 << DSL.BITS_DEX_LITE_DEX_VARIABLES_FEE)) |
            (fee_ << DSL.BITS_DEX_LITE_DEX_VARIABLES_FEE) |
            (revenueCut_ << DSL.BITS_DEX_LITE_DEX_VARIABLES_REVENUE_CUT);

        emit LogUpdateFeeAndRevenueCut(
            dexKey_,
            bytes8(dexId_),
            _dexVariables[dexId_],
            fee_,
            revenueCut_ * FOUR_DECIMALS
        );
    }
    ...
    function updateRebalancingStatus(DexKey calldata dexKey_, bool rebalancingStatus_) public _onlyDelegateCall {
        ...
        _dexVariables[dexId_] =
            (dexVariables_ & ~(X2 << DSL.BITS_DEX_LITE_DEX_VARIABLES_REBALANCING_STATUS)) |
            ((rebalancingStatus_ ? uint256(1) : uint256(0)) << DSL.BITS_DEX_LITE_DEX_VARIABLES_REBALANCING_STATUS);

        emit LogUpdateRebalancingStatus(dexKey_, bytes8(dexId_), _dexVariables[dexId_], rebalancingStatus_);
    }
    ...
    function updateRangePercents(
        ...
        _dexVariables[dexId_] =
            (dexVariables_ & ~(X28 << DSL.BITS_DEX_LITE_DEX_VARIABLES_UPPER_PERCENT)) |
        ...
        emit LogUpdateRangePercents(
            dexKey_,
            bytes8(dexId_),
            _dexVariables[dexId_],
            _rangeShift[dexId_],
            upperPercent_ * TWO_DECIMALS,
            lowerPercent_ * TWO_DECIMALS,
            shiftTime_
        );
    }
    ...
    function updateShiftTime(DexKey calldata dexKey_, uint256 shiftTime_) public _onlyDelegateCall {
        ...
        _centerPriceShift[dexId_] =
            (_centerPriceShift[dexId_] & ~(X24 << DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_SHIFTING_TIME)) |
            (shiftTime_ << DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_SHIFTING_TIME);

        emit LogUpdateShiftTime(dexKey_, bytes8(dexId_), _centerPriceShift[dexId_], shiftTime_);
    }
    ...
    function updateCenterPriceLimits(
        DexKey calldata dexKey_,
        uint256 maxCenterPrice_,
        uint256 minCenterPrice_
    ) public _onlyDelegateCall {
        bytes32 dexId_ = keccak256(abi.encode(dexKey_));
        uint256 dexVariables_ = _dexVariables[dexId_];
        if (dexVariables_ == 0) {
        ...
        _centerPriceShift[dexId_] =
            (_centerPriceShift[dexId_] & ~(X56 << DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_MAX_CENTER_PRICE)) |
            (BM.toBigNumber(maxCenterPrice_, SMALL_COEFFICIENT_SIZE, DEFAULT_EXPONENT_SIZE, BM.ROUND_UP) <<
                DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_MAX_CENTER_PRICE) |
            (BM.toBigNumber(minCenterPrice_, SMALL_COEFFICIENT_SIZE, DEFAULT_EXPONENT_SIZE, BM.ROUND_DOWN) <<
                DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_MIN_CENTER_PRICE);

        emit LogUpdateCenterPriceLimits(
            dexKey_,
            bytes8(dexId_),
            _centerPriceShift[dexId_],
            maxCenterPrice_,
    ...
    function updateThresholdPercent(
        ...
        _dexVariables[dexId_] =
            ...
            ((shiftTime_ > 0 ? uint256(1) : uint256(0)) <<
        ...
        uint oldUpperThresholdPercent_ = (dexVariables_ >>
            DSL.BITS_DEX_LITE_DEX_VARIABLES_UPPER_SHIFT_THRESHOLD_PERCENT) & X7;
        uint oldLowerThresholdPercent_ = (dexVariables_ >>
        ...
        emit LogUpdateThresholdPercent(
            ...
            _dexVariables[dexId_],
            _thresholdShift[dexId_],
            upperThresholdPercent_ * FOUR_DECIMALS,
            lowerThresholdPercent_ * FOUR_DECIMALS,
            shiftTime_
        );
    }
    ...
    function updateCenterPriceAddress(
        ...
        if (centerPriceAddress_ > 0) {
            ...
            _centerPriceShift[dexId_] =
                (_centerPriceShift[dexId_] & ~(X73 << DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_PERCENT)) |
            ...
            _dexVariables[dexId_] = (_dexVariables[dexId_] &
                ~(X19 << DSL.BITS_DEX_LITE_DEX_VARIABLES_CENTER_PRICE_CONTRACT_ADDRESS));

            _centerPriceShift[dexId_] =
                _centerPriceShift[dexId_] &
                ~(X73 << DSL.BITS_DEX_LITE_CENTER_PRICE_SHIFT_PERCENT);
        }

        emit LogUpdateCenterPriceAddress(
            dexKey_,
            bytes8(dexId_),
            _dexVariables[dexId_],
            _centerPriceShift[dexId_],
    ...
    function deposit(
        ...
        if (token0Decimals_ > TOKENS_DECIMALS_PRECISION)
            token0Amount_ /= 10 ** (token0Decimals_ - TOKENS_DECIMALS_PRECISION);
        ...
        if (token1Decimals_ > TOKENS_DECIMALS_PRECISION)
            token1Amount_ /= 10 ** (token1Decimals_ - TOKENS_DECIMALS_PRECISION);
        else token1Amount_ *= 10 ** (TOKENS_DECIMALS_PRECISION - token1Decimals_);

        uint256 token0TotalSupply_ = ((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_0_TOTAL_SUPPLY_ADJUSTED) &
            X60) + token0Amount_;
        uint256 token1TotalSupply_ = ((dexVariables_ >> DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_1_TOTAL_SUPPLY_ADJUSTED) &
            X60) + token1Amount_;

        dexVariables_ =
            (dexVariables_ & ~(X120 << DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_0_TOTAL_SUPPLY_ADJUSTED)) |
        ...
        emit LogDeposit(dexKey_, bytes8(dexId_), dexVariables_, token0Amount_, token1Amount_);
    }
    ...
    function withdraw(
        ...
        emit LogWithdraw(dexKey_, bytes8(dexId_), dexVariables_, token0Amount_, token1Amount_);
    }
