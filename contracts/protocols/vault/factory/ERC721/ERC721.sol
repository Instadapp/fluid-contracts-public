...
abstract contract ERC721 is Error {
    ...
    */
    ...
    function approve(address spender_, uint256 id_) public virtual {
        ...
        // nonexistent id, else the auth check below would report it as Unauthorized
        if (owner_ == address(0)) revert FluidVaultError(ErrorTypes.ERC721__InvalidParams);
    ...
    function safeTransferFrom(address from_, address to_, uint256 id_) public virtual {
        ...
        _checkOnERC721Received(to_, from_, id_, "");
    }
    ...
    function safeTransferFrom(address from_, address to_, uint256 id_, bytes calldata data_) public virtual {
        transferFrom(from_, to_, id_);
        _checkOnERC721Received(to_, from_, id_, data_);
    }

    /// @dev receiver safety check shared by `safeTransferFrom` and permissioned gated transfers that call
    ///      `ERC721.transferFrom` directly (their public `transferFrom` is blocked).
    function _checkOnERC721Received(address to_, address from_, uint256 id_, bytes memory data_) internal {
        if (
            !((to_.code.length == 0) ||
