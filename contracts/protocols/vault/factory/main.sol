...
abstract contract VaultFactoryAuth is VaultFactoryCore {
    ...
    function spell(address target_, bytes memory data_) external virtual onlyOwner returns (bytes memory response_) {
        assembly {
...
/// @dev implements VaultFactory deploy vault related helpers.
abstract contract VaultFactoryDeploymentCore is VaultFactoryCore, VaultFactoryAuth {
    ...
    function _deployVaultStart(address vaultDeploymentLogic_) internal returns (address vault_, uint256 vaultId_) {
        ...
        // Revert if vaultDeploymentLogic_ is not allowlisted.
        ...
        vaultId_ = ++_totalVaults;

        // compute vault address for vault id.
        vault_ = getVaultAddress(vaultId_);
    }

    function _deployVaultFinalize(address vault_, uint256 vaultId_, bool success_, bytes memory data_) internal {
...
/// @dev implements VaultFactory public deploy vault method.
abstract contract VaultFactoryDeployment is VaultFactoryDeploymentCore {
    ...
    function deployVault(
    ...
    ) public virtual returns (address vault_) {
        uint256 vaultId_;
        (vault_, vaultId_) = _deployVaultStart(vaultDeploymentLogic_);
        ...
        _deployVaultFinalize(vault_, vaultId_, success_, data_);
    }
}

abstract contract VaultFactoryERC721 is VaultFactoryCore, VaultFactoryDeploymentCore {
