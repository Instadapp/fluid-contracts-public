...
abstract contract Internals is FluidVaultOperate {
    function _colOperateBefore(
        ...
        if (colSharesMinMax_ > 0) {
            ...
            shares_ = int(
                SUPPLY.deposit{
                    value: (SUPPLY_TOKEN0 == NATIVE_TOKEN) ? uint(newColToken0_) : (SUPPLY_TOKEN1 == NATIVE_TOKEN)
                        ? uint(newColToken1_)
                        : 0
    ...
    function _debtOperateBefore(
        ...
        if (debtSharesMinMax_ > 0) {
            ...
            shares_ = -int(
                BORROW.payback{
                    value: (BORROW_TOKEN0 == NATIVE_TOKEN) ? uint(-newDebtToken0_) : (BORROW_TOKEN1 == NATIVE_TOKEN)
                        ? uint(-newDebtToken1_)
                        : 0
    ...
    function _colOperatePerfectBefore(
        ...
        (uint token0Amt_, uint token1Amt_) = SUPPLY.depositPerfect{
            value: (SUPPLY_TOKEN0 == NATIVE_TOKEN) ? uint(colToken0MinMax_) : (SUPPLY_TOKEN1 == NATIVE_TOKEN)
    ...
    function _debtOperatePerfectPayback(
        ...
        if (debtToken0MinMax_ < 0 && debtToken1MinMax_ < 0) {
            (token0Amt_, token1Amt_) = BORROW.paybackPerfect{
                value: (BORROW_TOKEN0 == NATIVE_TOKEN) ? uint(-debtToken0MinMax_) : (BORROW_TOKEN1 == NATIVE_TOKEN)
