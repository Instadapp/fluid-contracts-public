...
import { IFluidOracle } from "../../../../oracleV1_DEPRECATED/fluidOracle.sol";
import { IFluidOracleWrite } from "../../../../oracleV2/interfaces/iFluidOracleWrite.sol";
...
///         Deployed by "VaultFactory" and linked together with VaultT1 AdminModule `_adminImplementation()` and
///         FluidVaultT1Secondary (main2.sol) `_secondaryImplementation()`.
...
contract FluidVaultT1 is Helpers {
        ...
        )
    {
...

        {
            ...
            if (nftId_ == 0) {
                ...
                nftId_ = _vaultFactory().mint(_vaultId(), msg.sender);
                ...
                if ((newCol_ < 0 || newDebt_ > 0) && (_vaultFactory().ownerOf(nftId_) != msg.sender)) {
            ...
            if (newCol_ < 0 || newDebt_ > 0) {
                ...
                unchecked {
                    ...
                    if (
                        ...
                        {
                            address oracle_ = address(uint160(o_.vaultVariables2 >> 96));
                            // Prefer Write path when supported (e.g. CLX cache warm).
                            // Empty revert data => treat as unsupported selector and fall back to view.
                            // Non-empty revert data => write method existed and failed; rethrow.
                            try IFluidOracleWrite(oracle_).getExchangeRateOperateWrite() returns (uint256 rate_) {
                                temp_ = rate_;
                            } catch (bytes memory reason_) {
                                if (reason_.length == 0) {
                                    temp_ = IFluidOracle(oracle_).getExchangeRateOperate();
                                } else {
                                    assembly {
        ...
        if (newCol_ < 0) {
            ...
            if (temp_ > 0) {
                ...
                o_.userSupplyLiquidityData = _liquidity().readFromStorage(_liquidityUserSupplySlot());
...

        {
            ...
            if (newCol_ > 0) {
                // deposit
                _liquidity().operate{ value: _supplyToken() == NATIVE_TOKEN ? msg.value : 0 }(
                    _supplyToken(),
                    newCol_,
                    0,
                    address(0),
            ...
            if (newDebt_ < 0) {
                ...
                // payback
                _liquidity().operate{ value: temp_ }(
                    _borrowToken(),
                    0,
                    newDebt_,
                    address(0),
            ...
            if (newCol_ < 0) {
                // withdraw
                _liquidity().operate(_supplyToken(), newCol_, 0, to_, address(0), new bytes(0));
            }
            if (newDebt_ > 0) {
                // borrow
                _liquidity().operate(_borrowToken(), 0, newDebt_, address(0), to_, new bytes(0));
            }
        }
    ...
    function liquidate(
        ...
        address to_,
        bool absorb_
    ) public payable virtual returns (uint actualDebtAmt_, uint actualColAmt_) {
...

        {
            ...
            // temp_ -> debtPerCol
            {
                address oracle_ = address(uint160(memoryVars_.vaultVariables2 >> 96));
                // Prefer Write path when supported (e.g. CLX cache warm).
                // Empty revert data => treat as unsupported selector and fall back to view.
                // Non-empty revert data => write method existed and failed; rethrow.
                try IFluidOracleWrite(oracle_).getExchangeRateLiquidateWrite() returns (uint256 rate_) {
                    temp_ = rate_;
                } catch (bytes memory reason_) {
                    if (reason_.length == 0) {
                        temp_ = IFluidOracle(oracle_).getExchangeRateLiquidate(); // Price in 27 decimals
                    } else {
                        assembly {
                            revert(add(reason_, 0x20), mload(reason_))
                        }
        ...
        unchecked {
            ...
            _liquidity().operate(_supplyToken(), -int(actualColAmt_), 0, to_, address(0), new bytes(0));
        }
    ...
    function rebalance() external payable returns (int supplyAmt_, int borrowAmt_) {
        (supplyAmt_, borrowAmt_) = abi.decode(_spell(_secondaryImplementation(), msg.data), (int, int));
    }
    ...
    function liquidityCallback(address token_, uint amount_, bytes calldata data_) external {
        if (msg.sender != address(_liquidity()))
        ...
        SafeTransfer.safeTransferFrom(token_, abi.decode(data_, (address)), address(_liquidity()), amount_);
    }
    ...
    constructor(ConstantViews memory constants_) Helpers(constants_) {
        ...
        _initializeVaultState();
    }

    /// @dev initializes the vault storage state (branch + initial exchange prices). Extracted from the constructor
    /// into a `virtual` hook so that proxy-based deployments (e.g. the permissioned beacon stack) can skip it during
    /// the shared implementation deployment and instead run it per-instance from their `initialize()` method.
    /// For the standard, per-instance deployed vault this runs once from the constructor (unchanged behaviour).
    function _initializeVaultState() internal virtual {
        // Setting branch in vault.
        vaultVariables = (vaultVariables) | (1 << 22) | (1 << 52);

        uint liqSupplyExchangePrice_ = (_liquidity().readFromStorage(_liquiditySupplyExchangePriceSlot()) >>
            LiquiditySlotsLink.BITS_EXCHANGE_PRICES_SUPPLY_EXCHANGE_PRICE) & X64;
        uint liqBorrowExchangePrice_ = (_liquidity().readFromStorage(_liquidityBorrowExchangePriceSlot()) >>
    ...
    fallback() external {
        if (!(_vaultFactory().isGlobalAuth(msg.sender) || _vaultFactory().isVaultAuth(address(this), msg.sender))) {
        ...
        _spell(_adminImplementation(), msg.data);
    }
