...
import { IFluidLendingStaticRateModel } from "./iLendingStaticRateModel.sol";
...
interface IFTokenAdmin {
    ...
    ///         Callable by LendingFactory auths, or by the currently wired rewards rate model
    ///         (so `startRewards` can re-activate without permanent factory auth on the model).
    ...
    /// @notice updates the static rewards rate model contract.
    ///         Callable by LendingFactory auths, or by the currently wired rate model
    ///         (so `setStaticRate` can settle/re-wire without permanent factory auth on the model).
    ///         Wired-model callers may only set themselves or `address(0)`.
    /// @param staticRateModel_  the new static rewards rate model contract address.
    ///                          can be set to address(0) to disable rewards (to save gas)
    ...
    ///         When totalAssets() exceeds the Liquidity balance, deposits underlying from the rebalancer
    ///         (rewards direction). When the Liquidity balance exceeds totalAssets(), withdraws the
    ///         excess to the rebalancer (fees direction). Does not mint shares in either case.
    ///         Only callable by rebalancer.
    /// @return assets_ absolute amount deposited to or withdrawn from Liquidity
...
interface IFToken is IERC4626, IFTokenAdmin {
    ...
    function isStaticRateModelActive() external view returns (bool);
