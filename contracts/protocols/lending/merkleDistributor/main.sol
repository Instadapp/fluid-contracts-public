    ...
    FluidMerkleDistributorRewards
{
        ...
        Variables(params_.owner, params_.rewardToken)
    {
        ...
        if (block.chainid == 1) _blocksPerDistribution = _blocksPerDistribution / 12 seconds;
            ...
            _blocksPerDistribution = _blocksPerDistribution; // 1 second block time
    ...
    function _claim(
        ...
        if (vestingTime > 0) {
            uint256 vestingStartTime_ = uint256(vestingStartTime);
            uint256 vestingPeriod_ = block.timestamp > vestingStartTime_ ? block.timestamp - vestingStartTime_ : 0;
