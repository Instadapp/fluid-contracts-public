...
abstract contract LendingFactoryVariables is Owned, Error {
    /*//////////////////////////////////////////////////////////////
    ...
    /// @notice Returns all deployed fTokens.
    function allTokens() public view returns (address[] memory) {
        return _allTokens;
    }

    /// @notice Returns all available fToken types.
    function fTokenTypes() public view returns (string[] memory) {
        return _fTokenTypes;
    }

    /// @notice Returns the creation code for `fTokenType_`.
...
abstract contract LendingFactoryCore is LendingFactoryVariables, Events {
    ...
    /// @notice Sets whether `deployer_` is allowed to deploy fTokens.
    ...
    /// @notice Computes the deterministic fToken address for `asset_` and `fTokenType_` (CREATE3).
    /// @dev Same salt as `createToken`. Address is factory-specific because CREATE3 mixes in `address(this)`.
    ...
    /// @dev returns the SSTORE2 pointer for `fTokenType_` creation code.
    function _getFTokenCreationCodePointer(string calldata fTokenType_) internal view returns (address) {
        return _fTokenCreationCodePointers[keccak256(abi.encode(fTokenType_))];
    }
    ...
    /// @dev reverts if a token is already deployed for `salt_`, returning its deterministic address otherwise.
    function _requireTokenNotDeployed(bytes32 salt_) internal view returns (address token_) {
        token_ = CREATE3.getDeployed(salt_);
        if (Address.isContract(token_)) {
    ...
    /// @dev returns the asset address used for Liquidity slot lookups.
    function _getLiquiditySlotAsset(address asset_, bool isNativeUnderlying_) internal pure returns (address) {
        // native underlying always uses the native token at Liquidity, but also supports WETH
        return isNativeUnderlying_ ? _NATIVE_TOKEN_ADDRESS : asset_;
    }

    /// @dev reverts if fToken has not been configured at Liquidity contract yet.
    function _requireLiquidityConfigured(
        address asset_,
        bool isNativeUnderlying_
    ) internal view returns (bytes32 liquidityExchangePricesSlot_) {
        liquidityExchangePricesSlot_ = LiquiditySlotsLink.calculateMappingStorageSlot(
            LiquiditySlotsLink.LIQUIDITY_EXCHANGE_PRICES_MAPPING_SLOT,
            _getLiquiditySlotAsset(asset_, isNativeUnderlying_)
        );
    ...
    /// @dev deploys `creationCode_` with CREATE3 using `salt_`.
    function _deployToken(bytes32 salt_, bytes memory creationCode_) internal returns (address token_) {
        token_ = CREATE3.deploy(salt_, creationCode_, 0);
    }

    /// @dev stores the token in `allTokens` and emits its creation event.
    function _registerToken(address token_, address asset_, string calldata fTokenType_) internal {
        _allTokens.push(token_);
        emit LogTokenCreated(token_, asset_, _allTokens.length, fTokenType_);
    }
}

abstract contract LendingFactoryDeployment is LendingFactoryCore {
    /// @notice Sets or removes creation code for `fTokenType_`.
    function setFTokenCreationCode(string memory fTokenType_, bytes calldata creationCode_) external virtual onlyAuths {
    ...
    /// @notice Creates an fToken for `asset_` and `fTokenType_`.
    function createToken(
        address asset_,
        string calldata fTokenType_,
        bool isNativeUnderlying_
    ) external virtual validAddress(asset_) onlyDeployers returns (address token_) {
        address creationCodePointer_ = _getFTokenCreationCodePointer(fTokenType_);
        ...
        _requireTokenNotDeployed(salt_);
        _requireLiquidityConfigured(asset_, isNativeUnderlying_);
        ...
        _registerToken(token_, asset_, fTokenType_);
    }
}
...
contract FluidLendingFactory is LendingFactoryDeployment {
