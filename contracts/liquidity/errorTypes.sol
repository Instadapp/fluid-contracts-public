...
library ErrorTypes {
    ...
    /// @notice thrown if a token is not configured (exchangePricesAndConfig == 0)
    uint256 internal constant AdminModule__TokenNotDefined = 10029;

    /// @notice thrown if a token is tried to be unpaused but is not paused in the first place
    uint256 internal constant AdminModule__TokenNotPaused = 10030;
    ...
    /// @notice thrown when operateOnBehalfOf is called by non-auth and non-governance
    uint256 internal constant UserModule__OperateOnBehalfUnauthorized = 11019;

    /// @notice thrown when operateOnBehalfOf has supplyAmount < 0 (withdraw) or borrowAmount > 0 (borrow). Only deposit or payback allowed.
    uint256 internal constant UserModule__OperateOnBehalfDepositOrPaybackOnly = 11020;

    /// @notice thrown when operateOnBehalfOf is called with onBehalf_ as address(0)
    uint256 internal constant UserModule__OperateOnBehalfAddressZero = 11021;

    /// @notice thrown when operateOnBehalfOf is called with non-empty callbackData
    uint256 internal constant UserModule__OperateOnBehalfCallbackDataNotEmpty = 11022;

    /// @notice thrown when a token is paused
    uint256 internal constant UserModule__TokenPaused = 11023;
