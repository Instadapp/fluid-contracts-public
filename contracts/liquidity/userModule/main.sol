...
abstract contract CoreInternals is Error, CommonHelpers, Events {
    ...
    /// @param user_ address(0) = use msg.sender with pause checks; non-zero = use that address, skip pause checks.
    function _supplyOrWithdraw(
        ...
        uint256 userSupplyData_ = _userSupplyData[user_ == address(0) ? msg.sender : user_][token_];

        if (userSupplyData_ == 0) {
            revert FluidLiquidityError(ErrorTypes.UserModule__UserNotDefined);
        }
        if (user_ == address(0) && (userSupplyData_ >> LiquiditySlotsLink.BITS_USER_SUPPLY_IS_PAUSED) & 1 == 1) {
            revert FluidLiquidityError(ErrorTypes.UserModule__UserPaused);
        }

        // resolve user_ after pause check so storage write avoids the ternary (less stack pressure)
        if (user_ == address(0)) user_ = msg.sender;
    ...
    function _borrowOrPayback(
        ...
        uint256 userBorrowData_ = _userBorrowData[user_ == address(0) ? msg.sender : user_][token_];

        if (userBorrowData_ == 0) {
            revert FluidLiquidityError(ErrorTypes.UserModule__UserNotDefined);
        }
        if (user_ == address(0) && (userBorrowData_ >> LiquiditySlotsLink.BITS_USER_BORROW_IS_PAUSED) & 1 == 1) {
            revert FluidLiquidityError(ErrorTypes.UserModule__UserPaused);
        }

        // resolve user_ after pause check so borrow storage updates use the active caller slot
        if (user_ == address(0)) user_ = msg.sender;
...
abstract contract FluidLiquidityUserModule is CoreInternals {
    ...
    function operate(
        ...
        return
            _operateInternal(address(0), token_, supplyAmount_, borrowAmount_, withdrawTo_, borrowTo_, callbackData_);
    }

    /// @notice inheritdoc IFluidLiquidity
    /// @dev skipPauseCheck is true: deposits and paybacks intentionally bypass user pause status because
    ///      these operations reduce systemic risk (add collateral / reduce debt) and should always be possible.
    ///      Pause is designed to block withdrawals and borrows (value extraction), which are already forbidden here.
    function operateOnBehalfOf(
        ...
        if ((_isAuth[msg.sender] & 1) != 1 && _getGovernanceAddr() != msg.sender) {
            revert FluidLiquidityError(ErrorTypes.UserModule__OperateOnBehalfUnauthorized);
        }
        if (onBehalf_ == address(0)) {
            revert FluidLiquidityError(ErrorTypes.UserModule__OperateOnBehalfAddressZero);
        }
        if (supplyAmount_ < 0 || borrowAmount_ > 0) {
            revert FluidLiquidityError(ErrorTypes.UserModule__OperateOnBehalfDepositOrPaybackOnly);
        }
        if (callbackData_.length > 0) {
            revert FluidLiquidityError(ErrorTypes.UserModule__OperateOnBehalfCallbackDataNotEmpty);
        }
        (memVar3_, memVar4_) = _operateInternal(
            onBehalf_,
            token_,
            supplyAmount_,
            borrowAmount_,
        ...
        emit LogOperateOnBehalfOf(msg.sender, onBehalf_, token_, supplyAmount_, borrowAmount_, memVar3_, memVar4_);
    }

    /// @param user_ address(0) = use msg.sender with pause checks; non-zero = operate on that position and skip
    ///                    user-level pause checks. Non-zero user_ is a privileged path and must only come from
    ///                    `operateOnBehalfOf()` after auth / governance checks.
    function _operateInternal(
        address user_,
        address token_,
        int256 supplyAmount_,
        int256 borrowAmount_,
        ...
        address borrowTo_,
        bytes calldata callbackData_
    ) internal returns (uint256 memVar3_, uint256 memVar4_) {
        if (supplyAmount_ == 0 && borrowAmount_ == 0) {
        ...
        if (
            user_ == address(0) &&
            (o_.exchangePricesAndConfig >> LiquiditySlotsLink.BITS_EXCHANGE_PRICES_PAUSE_TOKEN) > 0
        ) {
            revert FluidLiquidityError(ErrorTypes.UserModule__TokenPaused);
        }
        ...
        if (supplyAmount_ != 0) {
            ...
            {
                memVar2_ = o_.totalAmounts; // totalAmountsBefore
        ...
        // emit Operate event
        emit LogOperate(
            user_ == address(0) ? msg.sender : user_,
            token_,
            supplyAmount_,
            borrowAmount_,
            withdrawTo_,
