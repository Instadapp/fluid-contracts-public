...
contract Events {
    ...
    /// @notice emitted when `operateOnBehalfOf()` is called. Enables tracking/alerting of on-behalf operations.
    /// @param operator the auth address (msg.sender) that triggered the on-behalf operation
    /// @param onBehalf the user address on whose behalf the operation is executed
    /// @param token token address for which this operation was executed
    /// @param supplyAmount supply (deposit) amount. if >0 then a deposit happened. if 0 then nothing.
    /// @param borrowAmount borrow (payback) amount. if <0 then a payback happened. if 0 then nothing.
    /// @param supplyExchangePrice updated supply exchange price (1e12)
    /// @param borrowExchangePrice updated borrow exchange price (1e12)
    event LogOperateOnBehalfOf(
