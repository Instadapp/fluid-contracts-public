...
abstract contract GuardianModule is AuthModule {
    ...
    /// @inheritdoc IFluidLiquidityAdmin
    function pauseTokens(address[] calldata tokens_) public onlyGuardians {
        uint256 length_ = tokens_.length;
        for (uint256 i; i < length_; ) {
            _checkIsContractOrNativeAddress(tokens_[i]);

            uint256 exchangePricesAndConfig_ = _exchangePricesAndConfig[tokens_[i]];
            if (exchangePricesAndConfig_ == 0) {
                revert FluidLiquidityError(ErrorTypes.AdminModule__TokenNotDefined);
            }

            _exchangePricesAndConfig[tokens_[i]] =
                exchangePricesAndConfig_ |
                (1 << LiquiditySlotsLink.BITS_EXCHANGE_PRICES_PAUSE_TOKEN);
    ...
    /// @inheritdoc IFluidLiquidityAdmin
    function unpauseTokens(address[] calldata tokens_) public onlyGuardians {
        uint256 length_ = tokens_.length;
        for (uint256 i; i < length_; ) {
            _checkIsContractOrNativeAddress(tokens_[i]);
            ...
            if (((exchangePricesAndConfig_ >> LiquiditySlotsLink.BITS_EXCHANGE_PRICES_PAUSE_TOKEN) & 1) == 0) {
                revert FluidLiquidityError(ErrorTypes.AdminModule__TokenNotPaused);
            }
