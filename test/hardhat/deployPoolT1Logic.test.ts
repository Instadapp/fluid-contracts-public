...
describe("PoolT1DeploymentLogic", () => {
  it("should deploy PoolT1DeploymentLogic", async () => {
    ...
    let res = await sstore2DeployerContract.deployCodeSplit(FluidDexT1OperationsCol__factory.bytecode, {
      gasLimit: 30000000,
    });
    let event = ((await res.wait())?.events as any as Event[])[0];
    const colOperations1 = (event as any).args[0];
    const colOperations2 = (event as any).args[1];

    res = await sstore2DeployerContract.deployCodeSplit(FluidDexT1OperationsDebt__factory.bytecode, {
      gasLimit: 30000000,
    });
    event = ((await res.wait())?.events as any as Event[])[0];
    const debtOperations1 = (event as any).args[0];
    const debtOperations2 = (event as any).args[1];
