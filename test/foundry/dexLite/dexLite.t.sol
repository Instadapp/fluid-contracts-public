...
contract DexLiteTest is LiquidityBaseTest {
    ...
    function dexCallback(address token_, uint256 amount_, bytes calldata data_) external {
        if (data_.length > 0) {
            ...
            if (shouldTestReentrancy_) {
                ...
                vm.expectRevert();
                DexKey memory dexKey = DexKey({ token0: address(USDC), token1: ETH_ADDRESS, salt: bytes32(0) });
                dexLite.swapSingle(dexKey, false, int256(0.01 ether), uint256(0), address(this), false, "", "");
            }
        }
    ...
    function testSwapUsdcToUsdt() public {
        ...
        DexKey memory dexKey = DexKey({ token0: token0, token1: token1, salt: bytes32(0) });
    ...
    function testMultiHopSwapEthUsdcUsdtEth() public {
        ...
        DexKey memory ethUsdcDexKey = DexKey({ token0: address(USDC), token1: ETH_ADDRESS, salt: bytes32(0) });
    ...
    function testErc20CallbackSwap() public {
        ...
        // Verify the swap results
        assertEq(
            v_.initialUsdcBalance - v_.finalUsdcBalance,
            v_.swapAmount,
            "Should spend exactly the swap amount of USDC"
        );
    ...
    function testFunctionSelectorOrder() public pure {
        ...
        assertEq(selectorInfos[0].functionName, "swapSingle", "swapSingle function should have the lowest selector");

        // Verify that swapHop has the second lowest selector
        assertEq(selectorInfos[1].functionName, "swapHop", "swapHop function should have the second lowest selector");
    }
    ...
    function _getReserves(DexKey memory dexKey) internal view returns (uint256 token0Reserve, uint256 token1Reserve) {
        ...
        uint256 token0ReserveAdjusted = (dexVariables >>
            DSL.BITS_DEX_LITE_DEX_VARIABLES_TOKEN_0_TOTAL_SUPPLY_ADJUSTED) & X60;
        uint256 token1ReserveAdjusted = (dexVariables >>
    ...
    function testSwapHopRevertsWithExtraDataZeroAddress() public {
        ...
        dexLite.swapHop{ value: 0.1 ether }(path, dexKeys, amountSpecified, amountLimits, transferParams);
    }
    ...
    function testSwapHopReentrancyProtection() public {
        ...
        uint256 amountOut = dexLite.swapHop(path, dexKeys, int256(100 * 1e6), amountLimits, transferParams);
    ...
    function testSwapHopEstimation() public {
        ...
        DexKey memory actualUsdcUsdtDexKey = DexKey({ token0: token0, token1: token1, salt: bytes32(uint256(501)) });
        ...
        DexKey memory estimateUsdcUsdtDexKey = DexKey({ token0: token0, token1: token1, salt: bytes32(uint256(503)) });
    ...
    function testWideRangeRebalancingEthUsdc() public {
        ...
        DexKey memory dexKey = DexKey({ token0: address(USDC), token1: ETH_ADDRESS, salt: bytes32(uint256(600)) });
        ...
        uint256 ethReceived = dexLite.swapSingle(dexKey, true, int256(usdcBuyAmount), 0, address(this), true, "", "");
