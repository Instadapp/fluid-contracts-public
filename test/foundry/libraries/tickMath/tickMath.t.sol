...
contract TickMathTest is Test {
    ...
    function call_getRatioAtTick(int tick) external pure {
        // used to fix catching revert at lower call level depth
        TickMath.getRatioAtTick(tick);
    }

    function test_getRatioAtTick_throwsForTooLow() public {
        vm.expectRevert();
        this.call_getRatioAtTick(TickMath.MIN_TICK - 1);
    }

    function test_getRatioAtTick_throwsForTooHigh() public {
        vm.expectRevert();
        this.call_getRatioAtTick(TickMath.MAX_TICK + 1);
    }
    ...
    function call_getTickAtRatio(uint256 ratioX96) external pure {
        // used to fix catching revert at lower call level depth
        TickMath.getTickAtRatio(ratioX96);
    }

    function test_getTickAtRatio_throwsForTooLow() public {
        vm.expectRevert();
        this.call_getTickAtRatio(TickMath.MIN_RATIOX96 - 1);
    }

    function test_getTickAtRatio_throwsForTooHigh() public {
        vm.expectRevert();
        this.call_getTickAtRatio(TickMath.MAX_RATIOX96 + 1);
    }
    ...
    function testFuzz_getRatioAtTick(int24 tick) public {
        ...
        // at MIN_TICK the result lands one below MIN_RATIOX96 — the LB-05 rounding asymmetry
        assertTrue(ratioX96 >= TickMath.MIN_RATIOX96 - 1 && ratioX96 <= TickMath.MAX_RATIOX96);
    }
