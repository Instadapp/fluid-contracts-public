...
import {
    ...
    Events,
    Structs
} from "../../../contracts/config/expandPercentHandler/main.sol";
...
abstract contract FluidExpandPercentConfigHandlerTests is LiquidityUserModuleBaseTest, Events {
    ...
    function test_rebalance() public {
        ...
        _supply(address(liquidity), mockProtocol, address(USDC), alice, 3 ether);
        ...
        _supply(address(liquidity), mockProtocol, address(USDC), alice, TVL_CHECKPOINT1);
        ...
        _supply(address(liquidity), mockProtocol, address(USDC), alice, TVL_CHECKPOINT2 - TVL_CHECKPOINT1);
        ...
        _supply(address(liquidity), mockProtocol, address(USDC), alice, TVL_CHECKPOINT3 - TVL_CHECKPOINT2);
        vm.prank(alice);
        configHandler.rebalance();
        ...
        _supply(address(liquidity), mockProtocol, address(USDC), alice, TVL_CHECKPOINT3); // total supply will be 83
        // trigger rebalance, should revert no update needed
        vm.expectRevert(
            abi.encodeWithSelector(Error.FluidConfigError.selector, ErrorTypes.ExpandPercentConfigHandler__NoUpdate)
        );
    ...
    function _withdrawSkippingLimits(uint256 amount) internal {
        ...
        _withdraw(address(liquidity), mockProtocol, address(USDC), alice, 1 ether);
        _withdraw(address(liquidity), mockProtocol, address(USDC), alice, amount);
