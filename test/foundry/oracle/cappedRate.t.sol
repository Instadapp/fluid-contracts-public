...
import { FluidCappedRate, FluidCappedRateBase } from "../../../contracts/oracleV2/cappedRates/fluidCappedRate.sol";
import { FluidWSTETHCappedRate, IWstETH } from "../../../contracts/oracleV2/cappedRates/implementations/wstethCappedRate.sol";
import { FluidERC4626CappedRate } from "../../../contracts/oracleV2/cappedRates/implementations/erc4626CappedRate.sol";
import { FluidERC46262xCappedRate } from "../../../contracts/oracleV2/cappedRates/implementations/erc46262xCappedRate.sol";
import { ErrorTypes } from "../../../contracts/oracleV2/cappedRates/errorTypes.sol";
import { Error } from "../../../contracts/oracleV2/cappedRates/error.sol";
...
contract FluidCappedRateTest is Test {
    ...
    function _erc4626Params(
        address rateSource_
    ) internal view returns (FluidCappedRateBase.CappedRateConstructorParams memory params_) {
        params_ = constructorParams;
        params_.infoName = "ASSET per 1 SHARE";
        params_.rateSource = rateSource_;
        params_.rateMultiplier = 1;
    }
    ...
    function test_erc4626ConvertHelpersFollowERC4626Direction() public {
        MockERC4626CappedRateSource source_ = new MockERC4626CappedRateSource(1.1e27);
        FluidERC4626CappedRate erc4626CappedRate_ = new FluidERC4626CappedRate(_erc4626Params(address(source_)));

        assertEq(erc4626CappedRate_.getExchangeRate(), 1.1e27);
        assertEq(erc4626CappedRate_.convertToAssets(1e27), 1.1e27);
        assertEq(erc4626CappedRate_.convertToShares(1.1e27), 1e27);
    }

    function test_erc46262xConvertHelpersFollowERC4626Direction() public {
        MockERC4626CappedRateSource source1_ = new MockERC4626CappedRateSource(1.1e27);
        MockERC4626CappedRateSource source2_ = new MockERC4626CappedRateSource(1.2e27);
        FluidERC46262xCappedRate erc46262xCappedRate_ = new FluidERC46262xCappedRate(
            _erc4626Params(address(source1_)),
            address(source2_)
        );

        assertEq(erc46262xCappedRate_.getExchangeRate(), 1.32e27);
        assertEq(erc46262xCappedRate_.convertToAssets(1e27), 1.32e27);
        assertEq(erc46262xCappedRate_.convertToShares(1.32e27), 1e27);
    }
    ...
    function test_maxYieldCapJumpsOnly1() public {
        ...
        // test with 1 yield jump, too much increase is not possible. at 10% APR, in 7 days the yield is 0.1918%
        uint256 newRate = (WSTETH_RATE_START * 100_30) / 100_00;
        vm.mockCall(WSTETH, abi.encodeWithSelector(IWstETH.stEthPerToken.selector), abi.encode(newRate));

        assertGt(cappedRate.configPercentDiff(), 0);
        assertApproxEqAbs(cappedRate.configPercentDiff(), 3000, 1); // 1e4 = 1%. we expect 0.3% diff

        cappedRate.rebalance(); // does not fully get there, limited by max yield

        (uint256 rate, uint256 maxReachedRate, , , , , , , , ) = cappedRate.getRatesAndCaps();
        assertEq(rate, newRate * 1e9);
        assertGt(rate, maxReachedRate);
    }
    ...
    function test_maxYieldCapJumps() public {
        ...
        // increase external rate now to valid 1 yield jump
        uint256 newRate = (WSTETH_RATE_START * 100_19) / 100_00;
        vm.mockCall(WSTETH, abi.encodeWithSelector(IWstETH.stEthPerToken.selector), abi.encode(newRate));

        assertGt(cappedRate.configPercentDiff(), 0);
        assertApproxEqAbs(cappedRate.configPercentDiff(), 1900, 1); // 1e4 = 1%. we expect 0.19% diff
        ...
        // increase external rate now. at 7 jumps * 0.19%, we can expect ~1.3% increase
        newRate = (newRate * 101_30) / 100_00;
        vm.mockCall(WSTETH, abi.encodeWithSelector(IWstETH.stEthPerToken.selector), abi.encode(newRate));

        assertGt(cappedRate.configPercentDiff(), 0);
        assertApproxEqAbs(cappedRate.configPercentDiff(), 13000, 1); // 1e4 = 1%. we expect 1.3% diff
...
contract MockERC4626CappedRateSource {
