...
contract FluidBuybackTest is Test, Events {
    ...
    function test_getOwner() public view {
        assertEq(IFluidBuyback(address(buybackProxy)).owner(), OWNER_ADDRESS);
    }

    function test_isRebalancer() public view {
        assertEq(IFluidBuyback(address(buybackProxy)).rebalancers(REBALANCER_ADDRESS), true);
    }
    ...
    function test_updateRebalancer() public {
        ...
        assertEq(IFluidBuyback(address(buybackProxy)).rebalancers(address(2)), true);
    }
    ...
    function test_collectFluidTokensToTreasury() public {
        ...
        assertEq(
            IERC20Upgradeable(FLUID_TOKEN_ADDRESS).balanceOf(TREASURY_ADDRESS),
