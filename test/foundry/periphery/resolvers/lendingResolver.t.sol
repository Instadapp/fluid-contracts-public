...
contract FluidLendingResolverTest is FluidLendingResolverTestBase {
    ...
    function test_getDeploymentName_defaultsEmptyOnProdFactory() public view {
        assertEq(lendingResolver.getDeploymentName(), "");
    }
    ...
    function test_getFTokenDetails() public {
        ...
        assertEq(details.accessType, 0); // production fToken has no ACCESS_TYPE() → PUBLIC default
        (
            ,
        ...
        assertEq(liquidityRate_, 0);
        assertEq(totalRate_, relativeModelRate_);
        ...
        FluidLendingResolverStructs.FTokenDetails memory expectedDetails = FluidLendingResolverStructs.FTokenDetails({
            ...
            relativeModelRate: relativeModelRate_,
            staticModelRate: 0,
            isStaticRate: false,
            rewardsActive: true,
            liquidityRate: 0,
            totalRate: relativeModelRate_,
        ...
        details = lendingResolver.getFTokenDetails(IFToken(address(lendingFToken)));
    ...
    function test_getFTokenInternalData() public {
        ...
        ) = lendingResolver.getFTokenInternalData(IFToken(address(lendingFToken)));
        ...
        for (uint256 i = 0; i < allDetails.length; i++) {
            ...
            FluidLendingResolverStructs.FTokenDetails memory expectedTokenDetails = FluidLendingResolverStructs
                .FTokenDetails({
                    ...
                    relativeModelRate: allDetails[i].relativeModelRate,
                    staticModelRate: allDetails[i].staticModelRate,
                    isStaticRate: allDetails[i].isStaticRate,
                    rewardsActive: allDetails[i].rewardsActive,
                    liquidityRate: allDetails[i].liquidityRate,
                    totalRate: allDetails[i].totalRate,
                    rebalanceDifference: allDetails[i].rebalanceDifference,
                    liquidityUserSupplyData: allDetails[i].liquidityUserSupplyData,
                    accessType: allDetails[i].accessType
                });
    ...
    function test_getFTokenRewards() public {
        ...
        assertEq(relativeModelRate_, 2000); // 20% at Liquidity scale (100 = 1%)
        assertEq(staticModelRate_, int256(0));
        assertFalse(isStaticRate_);
        assertTrue(rewardsActive_);
        ...
        assertEq(staticModelRate_, int256(details.staticModelRate));
        assertEq(rewardsActive_, details.rewardsActive);
    }
    ...
    function test_getFTokenRewardsRateModelConfig() public {
        ...
        (
            ...
            int256 actualMaxRateOrStaticRate_,
        ...
        assertEq(actualMaxRateOrStaticRate_, int256(50 * 1e12)); // streaming: 50% ceiling in on-chain model scale
        assertEq(actualInitiator_, alice);

        (, , , , uint256 onChainMaxRate_, , ) = rateModel.getConfig();
        assertEq(
            actualMaxRateOrStaticRate_,
            int256(onChainMaxRate_),
            "streaming path matches model getConfig().maxRate"
        );
    }
    ...
    function test_getUserPosition() public {
        ...
        // assert values change as expected
        userPosition = lendingResolver.getUserPosition(IFToken(address(lendingFToken)), alice);
    ...
    function test_getPreviews() public {
        ...
        ) = lendingResolver.getPreviews(IFToken(address(lendingFToken)), DEFAULT_AMOUNT, DEFAULT_AMOUNT);
    ...
    function assertFTokenDetails(
        ...
        assertEq(details.relativeModelRate, expectedDetails.relativeModelRate);
        assertEq(details.staticModelRate, expectedDetails.staticModelRate);
        assertEq(details.isStaticRate, expectedDetails.isStaticRate);
        assertEq(details.rewardsActive, expectedDetails.rewardsActive);
        assertEq(details.liquidityRate, expectedDetails.liquidityRate);
        assertEq(details.totalRate, expectedDetails.totalRate);
        assertEq(details.rebalanceDifference, expectedDetails.rebalanceDifference);
        assertEq(details.accessType, expectedDetails.accessType);
