...
contract FluidWalletFactoryTest is VaultT1BaseTest {
    ...
    function setUp() public virtual override {
        ...
        fluidWalletImplementation = new FluidWalletImplementation(address(vaultFactory), address(proxy));
    ...
    function test_RevertWhen_UpgradableOfImplementationNonOwner() public {
        address fluidWalletFactoryImplementationNew = address(
            new FluidWalletFactory(address(vaultFactory), address(fluidWalletFactory))
        );
    ...
    function test_RevertWhen_FlashloanActionsDueToOperation() public {
        address wallet_ = fluidWalletFactory.deploy(address(alice));
    ...
    function testVaultOperation() public {
        ...
        vm.prank(alice);
        (uint256 nftId_, , ) = vaultTwo.operate(0, collateral, debt, alice);

        FluidWalletImplementation.Action[] memory actions_ = new FluidWalletImplementation.Action[](2);
        actions_[0] = FluidWalletErrorsAndEvents.Action({
            target: address(DAI),
            data: abi.encodeWithSignature("approve(address,uint256)", address(vaultTwo), uint256(collateral)),
            operation: 0,
            value: 0
        });
        ...
        actions_[1] = FluidWalletErrorsAndEvents.Action({
            ...
            data: abi.encodeWithSelector(vaultTwo.operate.selector, nftId_, collateral, debt, wallet_),
            operation: 0,
            value: 0
        });
        ...
        vm.prank(alice);
        vaultFactory.safeTransferFrom(alice, address(fluidWalletFactory), nftId_, abi.encode(actions_));
    ...
    function testVaultOperationNativeDebt() public {
        ...
        actions_[0] = FluidWalletErrorsAndEvents.Action({
            target: address(USDC),
            data: abi.encodeWithSignature("approve(address,uint256)", address(vaultFour), uint256(collateral)),
            operation: 0,
            value: 0
        });
        ...
        actions_[1] = FluidWalletErrorsAndEvents.Action({
            ...
            data: abi.encodeWithSelector(vaultFour.operate.selector, nftId_, collateral, debt, wallet_),
            operation: 0,
            value: 0
        });
