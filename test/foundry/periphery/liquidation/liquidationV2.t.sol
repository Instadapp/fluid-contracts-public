...
contract VaultT1LiquidatorBase is VaultsBaseTest, VaultT1BaseTest {
    ...
    function _getLiquidationParams(
        ...
        if (params_.vaultType == VaultType.VaultT2 || params_.vaultType == VaultType.VaultT4) {
            (
                liquidatorData_.collateralAmountToken0,
                liquidatorData_.collateralAmountToken1
            ) = _getDexWithdrawPerfectInOneTokenAmount(var_.dex, liquidationData_.outAmt, params_.withdrawInToken0);

            liquidatorData_.sellToken = params_.withdrawInToken0
                ? liquidationData_.token0Out
                : liquidationData_.token1Out;
            liquidatorData_.sellAmount = params_.withdrawInToken0
                ? liquidatorData_.collateralAmountToken0
                : liquidatorData_.collateralAmountToken1;
            ...
            (liquidatorData_.collateralAmountToken0, liquidatorData_.collateralAmountToken1) = (
        ...
        if (params_.vaultType == VaultType.VaultT3 || params_.vaultType == VaultType.VaultT4) {
            (
                liquidatorData_.debtAmountToken0,
                liquidatorData_.debtAmountToken1
            ) = _getDexPaybackPerfectInOneTokenAmount(var_.dex, liquidationData_.inAmt, params_.paybackInToken0);

            address debtToken_ = params_.paybackInToken0 ? liquidationData_.token0In : liquidationData_.token1In;
            uint256 debtAmount_ = params_.paybackInToken0
                ? liquidatorData_.debtAmountToken0
                : liquidatorData_.debtAmountToken1;
    ...
    function _testLiquidator(LiquidatorParams memory params_, string memory customMessage_) internal {
        ...
        VaultStructs.LiquidationStruct memory liquidationData_ = vaultResolver.getVaultLiquidation(
            address(var_.vault),
            0
        );
        assertNotEq(
            liquidationData_.outAmt,
            0,
            string(abi.encodePacked(var_.assertMessage, "liquidationData_.outAmt: Before first liquidation"))
        );
        assertNotEq(
            liquidationData_.inAmt,
            0,
            string(abi.encodePacked(var_.assertMessage, "liquidationData_.inAmt: Before first liquidation"))
        );
        assertNotEq(
        ...
        assertNotEq(
            ...
            0,
            string(abi.encodePacked(var_.assertMessage, "liquidationData_.inAmtWithAbsorb: Before first liquidation"))
        );
        ...
        vm.prank(alice);
        vaultLiquidator.execute(
            address(vaultLiquidatorImplementationV1),
            abi.encodeWithSelector(VaultLiquidatorImplementationV1.liquidation.selector, liquidatorParams)
        );
        if (params_.vaultType != VaultType.VaultT1) {
            VaultDexStateData memory postState_ = _getDexVaultState(
                params_.vaultParams,
                params_.vaultType,
            ...
            assertApproxEqAbs(
                ...
                -liquidatorData_.totalToken0OutOfLiquidity,
                750294766000000,
            ...
            assertApproxEqAbs(
                ...
                -liquidatorData_.totalToken1OutOfLiquidity,
                750294766000000,
        ...
        assertApproxEqAbs(
            ...
            0,
            750294766000000,
            string(abi.encodePacked(var_.assertMessage, "liquidationData_.outAmt: After first liquidation"))
        );
        assertApproxEqAbs(
            liquidationData_.inAmt,
            0,
            750294766000000,
            string(abi.encodePacked(var_.assertMessage, "liquidationData_.inAmt: After first liquidation"))
        );
        assertApproxEqAbs(
            liquidationData_.outAmtWithAbsorb,
            0,
            750294766000000,
            string(abi.encodePacked(var_.assertMessage, "liquidationData_.outAmtWithAbsorb: After first liquidation"))
        );
        assertApproxEqAbs(
            liquidationData_.inAmtWithAbsorb,
            0,
            750294766000000,
            string(abi.encodePacked(var_.assertMessage, "liquidationData_.inAmtWithAbsorb: After first liquidation"))
        );
    }
        ...
        for (uint256 i = 0; i < vaults_.length; i++) {
            for (uint256 j = 0; j < vaultTypes_.length; j++) {
                ...
                OperateData memory d_ = _testOperate(
                    vaults_[i],
                    vaultTypes_[j],
                    _convertOperateParamsToWei(vaults_[i], vaultTypes_[j], param_),
                    alice,
                    0,
                    "testLiquidator(supply)"
                );
                ...
                LiquidatorParams memory liquidatorParams = LiquidatorParams({
                    ...
                    withdrawInToken0: vaultTypes_[j] == VaultType.VaultT4 ? false : true,
                    paybackInToken0: vaultTypes_[j] == VaultType.VaultT3 ? false : true
                });
