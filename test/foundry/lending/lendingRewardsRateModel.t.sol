...
/// @dev Spy fToken used to prove `startRewards` settles under the expired schedule before overwrite,
/// then calls `updateRewards` only after the new schedule is written.
contract FTokenStartRewardsSpy {
    FluidLendingRewardsRateModel public model;
    uint256 public oldEndTime;
    bool public sawUpdateRatesUnderOldSchedule;
    bool public sawUpdateRewardsUnderNewSchedule;

    function init(FluidLendingRewardsRateModel model_, uint256 oldEndTime_) external {
        model = model_;
        oldEndTime = oldEndTime_;
    }

    function updateRates() external returns (uint256, uint256) {
        (int256 rate_, bool ended_, , uint256 endTime_) = model.getRateV2(1e18);
        require(!sawUpdateRatesUnderOldSchedule, "updateRates should run once before overwrite");
        require(ended_, "first updateRates must see ended schedule");
        require(endTime_ == oldEndTime, "first updateRates must see old endTime");
        require(rate_ > 0, "getRateV2 keeps actual rate when ended");
        sawUpdateRatesUnderOldSchedule = true;
        return (1e12, 1e12);
    }

    function updateRewards(IFluidLendingRewardsRateModel) external {
        require(sawUpdateRatesUnderOldSchedule, "updateRates must run before updateRewards");
        (, bool ended_, uint256 startTime_, ) = model.getRateV2(1e18);
        require(!ended_, "updateRewards must see new live schedule");
        require(startTime_ == block.timestamp, "new schedule must already be written");
        sawUpdateRewardsUnderNewSchedule = true;
    }
}
...
contract FluidLendingRewardsRateModelTest is Test {
    ...
    function testGetRateV2() public {
        uint256 expectedEndTime = block.timestamp + DURATION;

        // below START_TVL: rate 0, not ended
        (int256 rate, bool ended, uint256 startTime, uint256 endTime) = model.getRateV2(START_TVL - 1);
        assertEq(rate, int256(0));
        assertEq(ended, false);
        assertEq(startTime, block.timestamp);
        assertEq(endTime, expectedEndTime);

        // active: same rate as legacy getRate, plus endTime
        (rate, ended, startTime, endTime) = model.getRateV2(START_TVL * 2);
        (uint256 legacyRate, bool legacyEnded, uint256 legacyStartTime) = model.getRate(START_TVL * 2);
        assertGt(rate, int256(0));
        assertEq(rate, int256(legacyRate));
        assertEq(ended, legacyEnded);
        assertEq(startTime, legacyStartTime);
        assertEq(endTime, expectedEndTime);

        // after duration: ended, but rate stays the actual phase rate (for exact tail accrual),
        // while legacy getRate returns 0 for old fTokens
        uint256 rateBeforeEnd = uint256(rate);
        vm.warp(block.timestamp + DURATION + 1);
        (rate, ended, startTime, endTime) = model.getRateV2(START_TVL * 2);
        assertEq(rate, int256(rateBeforeEnd), "getRateV2 returns the actual phase rate even when ended");
        assertEq(ended, true);
        assertEq(endTime, expectedEndTime);

        (legacyRate, legacyEnded, ) = model.getRate(START_TVL * 2);
        assertEq(legacyRate, 0, "legacy getRate returns 0 once ended");
        assertEq(legacyEnded, true);
    }
    ...
    function testStartRewards_revertsWhenQueuedPhaseExists() public {
        model.queueNextRewards(200e6, 14 days);
        ...
        model.startRewards(300e6, 30 days, block.timestamp);
    }

    function testStartRewards_reactivatesRewardsAfterNaturalEnd() public {
        (, , , uint256 oldEndTime_) = model.getRateV2(IFToken(address(fToken)).totalAssets());
        vm.warp(oldEndTime_ + 1);

        // first poke after expiry clears _rewardsActive on the fToken
        IFTokenAdmin(address(fToken)).updateRates();
        (, , , , , bool rewardsActiveBefore_, , , ) = IFToken(address(fToken)).getData();
        assertFalse(rewardsActiveBefore_, "expired schedule poke clears rewardsActive");

        model.startRewards(300e6, 30 days, block.timestamp);

        (, , , , , bool rewardsActiveAfter_, , , ) = IFToken(address(fToken)).getData();
        assertTrue(rewardsActiveAfter_, "startRewards must re-wire via updateRewards after overwrite");
    }

    function testStartRewards_settlesExpiredTailBeforeOverwrite() public {
        // Production fTokens at this fork still use legacy getRate (rate forced 0 once ended), so an
        // exchange-price equality cannot prove endTime-tail settlement. Use a spy to assert ordering:
        // updateRates under the expired schedule → write new schedule → updateRewards under the new one.
        // Exact rewards-tail economics are covered by fTokenRewardsRateModelV2Fork Part B.
        FTokenStartRewardsSpy spy_ = new FTokenStartRewardsSpy();
        FluidLendingRewardsRateModel spyModel_ = new FluidLendingRewardsRateModel(
            address(this),
            address(spy_),
            address(0),
            address(0),
        ...
        uint256 oldEndTime_ = block.timestamp + DURATION;
        spy_.init(spyModel_, oldEndTime_);

        vm.warp(oldEndTime_ + 1);
        spyModel_.startRewards(300e6, 30 days, block.timestamp);

        assertTrue(spy_.sawUpdateRatesUnderOldSchedule(), "must settle under expired schedule first");
        assertTrue(spy_.sawUpdateRewardsUnderNewSchedule(), "must rewire only after new schedule write");

        (uint256 duration_, uint256 startTime_, , , , uint256 rewardAmount_, ) = spyModel_.getConfig();
        assertEq(duration_, 30 days);
        assertEq(startTime_, block.timestamp);
        assertEq(rewardAmount_, 300e6);
    }
