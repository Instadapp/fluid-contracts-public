...
abstract contract fTokenBaseSetUp is LiquidityBaseTest {
    ...
    function setUp() public virtual override {
        ...
        _supply(address(liquidity), mockProtocol, address(underlying), alice, DEFAULT_AMOUNT);
    }
...
abstract contract fTokenBaseActionsTest is fTokenBaseSetUp, fTokenEvents {
    ...
    //     _withdraw(address(liquidity), mockProtocol, address(underlying), alice, DEFAULT_AMOUNT);
    ...
    function test_rebalance_EmitLogRebalance() public virtual {
        ...
        //check event: rewards/deposit path → positive (same sign as legacy absolute LogRebalance)
        vm.expectEmit(true, true, true, true);
        emit LogRebalance(int256(DEFAULT_AMOUNT / 5));
    ...
    function test_updateRates_RevertIfNewLiquidityExchangePriceSmallerThanOld() public {
        ...
        fTokenHarness fTokenHarness_ = new fTokenHarness(liquidityProxy, IFluidLendingFactory(address(factory)), USDC);
        vm.expectRevert(
    ...
    function test_maxDeposit_WithDeposits() public virtual {
        ...
        lendingFToken.deposit(DEFAULT_AMOUNT, alice);
        vm.prank(alice);
        lendingFToken.deposit(1e24, alice);
        vm.prank(alice);
        lendingFToken.deposit(1e28, alice);
        vm.prank(alice);
        lendingFToken.deposit(1e32, alice);
        vm.prank(alice);
        lendingFToken.deposit(1e36, alice);

        uint256 maxDeposit = lendingFToken.maxDeposit(address(0));
        assertApproxEqAbs(
            maxDeposit,
            uint256(uint128(type(int128).max)) - DEFAULT_AMOUNT * 2 - 1e24 - 1e28 - 1e32 - 1e36,
            2e20 // offset of current deposited amounts of BigMath
        );
    ...
    function test_maxMint_WithDeposits() public virtual {
        ...
        uint256 maxMint = lendingFToken.maxMint(address(0));
        uint256 expectedMaxMint = uint256(uint128(type(int128).max)) - DEFAULT_AMOUNT * 2 - 1e24 - 1e28 - 1e32 - 1e36;
        assertApproxEqAbs(
            maxMint,
            expectedMaxMint,
            2e20 // offset of current deposited amounts of BigMath
    ...
    function test_maxWithdraw_WithWithdrawalLimit() public virtual {
        ...
        _resetDecay(address(liquidity), address(lendingFToken), address(underlying));
