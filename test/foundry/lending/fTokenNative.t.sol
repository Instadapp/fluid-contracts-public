...
import { fToken, fTokenBase } from "../../../contracts/protocols/lending/fToken/main.sol";
...
contract fTokenNativeETHActionsTest is fTokenNativeTestBase, fTokenEvents {
    ...
    function test_withdrawNative() public {
        vm.prank(alice);
        lendingFTokenNative.depositNative{ value: DEFAULT_AMOUNT }(alice);
        assertNotEq(lendingFToken.balanceOf(alice), 0);
...
contract fTokenNativeUnderlyingExposed is fTokenNativeUnderlyingOverrides {
    ...
    function liquidityCallback(address, uint256, bytes calldata) external pure override(fTokenBase) {
        revert("Not implemented");
    }
