...
contract LiquidityAdminModuleUserWithdrawalLimitTest is LiquidityAdminModuleBaseTest {
    function setUp() public virtual override {
        ...
        _withdraw(address(liquidity), mockProtocol, address(USDC), alice, 0.2 ether);
    ...
    function test_updateUserWithdrawalLimit_GoesBelowBaseLimit() public {
        ...
        // withdraw max, now withdrawable would be 0
        _withdraw(address(liquidity), mockProtocol, address(USDC), alice, 0.14 ether);
        (userSupplyData, ) = resolver.getUserSupplyData(address(mockProtocol), address(USDC));
        ...
        _withdraw(address(liquidity), mockProtocol, address(USDC), alice, 0.06 ether);
        (userSupplyData, ) = resolver.getUserSupplyData(address(mockProtocol), address(USDC));
...
contract LiquidityAdminModuleCollectRevenueTest is LiquidityAdminModuleBaseTest {
    ...
    function test_AdminCollectRevenue() public {
        ...
        // 1. supply as alice
        _supply(address(liquidity), mockProtocol, address(USDC), alice, 10000 ether);
    ...
    function test_AdminCollectRevenueNoSuppliers() public {
        ...
        _withdraw(address(liquidity), mockProtocol, address(USDC), alice, 10760 ether);
...
contract LiquidityAdminModulePauseTokenTest is LiquidityAdminModuleBaseTest {
    function setUp() public override {
        super.setUp();
    ...
    function test_GuardianPauseToken() public {
        uint256 exchangePricesAndConfig_ = resolver.getExchangePricesAndConfig(address(USDC));
        assertEq((exchangePricesAndConfig_ >> 255) & 1, 0);

        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);

        vm.prank(alice);
        GuardianModule(address(liquidity)).pauseTokens(tokens_);

        exchangePricesAndConfig_ = resolver.getExchangePricesAndConfig(address(USDC));
        assertEq((exchangePricesAndConfig_ >> 255) & 1, 1);
    }

    function test_GuardianPauseToken_MultipleTokens() public {
    ...
    function test_GuardianPauseToken_AlreadyPaused() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);
    ...
    function test_GuardianPauseToken_PreservesOtherBits() public {
        uint256 exchangePricesAndConfigBefore_ = resolver.getExchangePricesAndConfig(address(USDC));
        ...
        uint256 exchangePricesAndConfigAfter_ = resolver.getExchangePricesAndConfig(address(USDC));

        // all bits except bit 255 should be unchanged
        uint256 mask_ = 0x7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
        assertEq(exchangePricesAndConfigBefore_ & mask_, exchangePricesAndConfigAfter_ & mask_);
    }

    function test_GuardianPauseToken_LogPauseToken() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);

        vm.expectEmit(false, false, false, true);
        emit LogPauseToken(tokens_);

        vm.prank(alice);
        GuardianModule(address(liquidity)).pauseTokens(tokens_);
    }

    function test_GuardianPauseToken_RevertUnauthorized() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);

        vm.expectRevert(
            abi.encodeWithSelector(Error.FluidLiquidityError.selector, ErrorTypes.AdminModule__OnlyGuardians)
        );
    ...
    function test_GuardianPauseToken_RevertTokenNotDefined() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(resolver); // not a configured token

        vm.expectRevert(
            abi.encodeWithSelector(Error.FluidLiquidityError.selector, ErrorTypes.AdminModule__TokenNotDefined)
        );
    ...
    function test_GuardianPauseToken_GovernanceCanCall() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);
    ...
    function test_GuardianUnpauseToken() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);
        ...
        // unpause
        vm.prank(alice);
        GuardianModule(address(liquidity)).unpauseTokens(tokens_);

        exchangePricesAndConfig_ = resolver.getExchangePricesAndConfig(address(USDC));
        assertEq((exchangePricesAndConfig_ >> 255) & 1, 0);
    }

    function test_GuardianUnpauseToken_MultipleTokens() public {
        address[] memory tokens_ = new address[](2);
        tokens_[0] = address(USDC);
        tokens_[1] = address(DAI);
    ...
    function test_GuardianUnpauseToken_PreservesOtherBits() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);
        ...
        assertEq(exchangePricesAndConfigBefore_, exchangePricesAndConfigAfter_);
    }

    function test_GuardianUnpauseToken_LogUnpauseToken() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);
        ...
        vm.expectEmit(false, false, false, true);
        emit LogUnpauseToken(tokens_);

        vm.prank(alice);
        GuardianModule(address(liquidity)).unpauseTokens(tokens_);
    }

    function test_GuardianUnpauseToken_RevertUnauthorized() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);
    ...
    function test_GuardianUnpauseToken_RevertTokenNotPaused() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);

        vm.expectRevert(
            abi.encodeWithSelector(Error.FluidLiquidityError.selector, ErrorTypes.AdminModule__TokenNotPaused)
        );
    ...
    function test_GuardianUnpauseToken_RevertTokenNotDefined() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(resolver); // not a configured token
    ...
    function test_GuardianUnpauseToken_GovernanceCanCall() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = address(USDC);

        // pause first as guardian
        vm.prank(alice);
        GuardianModule(address(liquidity)).pauseTokens(tokens_);

        // unpause as governance
        vm.prank(admin);
        GuardianModule(address(liquidity)).unpauseTokens(tokens_);
    ...
    function test_GuardianPauseToken_NativeToken() public {
        address[] memory tokens_ = new address[](1);
        tokens_[0] = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;

        vm.prank(alice);
        GuardianModule(address(liquidity)).pauseTokens(tokens_);

        uint256 exchangePricesAndConfig_ = resolver.getExchangePricesAndConfig(
        ...
        exchangePricesAndConfig_ = resolver.getExchangePricesAndConfig(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);
        assertEq((exchangePricesAndConfig_ >> 255) & 1, 0);
    }
}
