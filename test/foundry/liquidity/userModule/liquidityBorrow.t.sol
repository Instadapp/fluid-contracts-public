...
contract LiquidityUserModuleBorrowTests is LiquidityUserModuleBaseTest {
    ...
    function test_operate_RevertBorrowOperateAmountRatioExcess() public {
        ...
        // execute operate
        vm.prank(alice);
        mockProtocol.operate(address(USDC), int256(0), 1.1e26, address(0), alice, abi.encode(alice));
    ...
    function test_operate_RevertIfBorrowMoreThanAvailableBalance() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                SafeTransfer.FluidSafeTransferError.selector,
                LibsErrorTypes.SafeTransfer__TransferFailed
            )
        );
...
contract LiquidityUserModuleBorrowTestsWithInterest is LiquidityUserModuleBaseTest {
    ...
    function test_operate_BorrowWhenUtilizationAbove100Percent() public {
        ...
        _supply(address(liquidity), mockProtocol, address(USDC), alice, 50 ether);
