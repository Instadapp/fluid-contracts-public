...
contract LiquidityUserModuleWithdrawTestSuiteNative is LiquidityUserModuleOperateTestSuite {
    function setUp() public virtual override {
        ...
        _supplyNative(address(liquidity), mockProtocol, alice, DEFAULT_SUPPLY_AMOUNT);
...
contract LiquidityUserModuleWithdrawTests is LiquidityUserModuleBaseTest {
    ...
    function test_operate_RevertWithdrawOperateAmountRatioExcess() public {
        ...
        vm.prank(alice);
        mockProtocol.operate(address(USDC), 1e22, int256(0), address(0), address(0), abi.encode(alice));
        vm.prank(alice);
        mockProtocol.operate(address(USDC), 1e26, int256(0), address(0), address(0), abi.encode(alice));
        vm.prank(alice);
        mockProtocol.operate(address(USDC), 1e30, int256(0), address(0), address(0), abi.encode(alice));

        _resetDecay(address(liquidity), address(mockProtocol), address(USDC));

        vm.expectRevert(
            abi.encodeWithSelector(Error.FluidLiquidityError.selector, ErrorTypes.UserModule__OperateAmountRatioExcess)
        );

        // execute operate
        vm.prank(alice);
        mockProtocol.operate(address(USDC), -int256(54e28), int256(0), alice, address(0), abi.encode(alice));

        vm.prank(alice);
        mockProtocol.operate(address(USDC), -int256(45e28), int256(0), alice, address(0), abi.encode(alice));
    }
    ...
    function test_operate_RevertIfWithdrawMoreThanSupplied() public {
        ...
        _withdraw(address(liquidity), mockProtocol, address(USDC), alice, DEFAULT_SUPPLY_AMOUNT + 1);
    }

    function test_operate_RevertIfWithdrawMoreThanSuppliedNative() public {
        vm.expectRevert(stdError.arithmeticError);
        _withdrawNative(address(liquidity), mockProtocol, alice, DEFAULT_SUPPLY_AMOUNT + 1);
    }
}
...
contract LiquidityUserModuleWithdrawTestsInterestFree is LiquidityUserModuleWithdrawTests {
    ...
    function test_operate_WithdrawExactToZero() public {
        ...
        _supply(address(liquidity), mockProtocolWithInterest, address(USDC), alice, DEFAULT_SUPPLY_AMOUNT);
        ...
        _withdraw(address(liquidity), mockProtocol, address(USDC), alice, userSupplyData.supply);
