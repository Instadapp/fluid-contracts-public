...
import { FluidLiquidityUserModuleMainnet } from "../../../contracts/liquidity/userModule/mainMainnet.sol";
...
import { FluidLiquidityAdminModuleMainnet } from "../../../contracts/liquidity/adminModule/mainMainnet.sol";
...
abstract contract LiquidityBaseTest is Test, TestHelpers {
    ...
    function setUp() public virtual {
        ...
        liquidity = new FluidLiquidityProxy(admin, address(1)); // non-zero dummy required to avoid sig-slot collision

        // To run the "others" test suite, set the environment variable RUN_OTHERS to true:
        // Example: RUN_OTHERS=true forge test -v
        // By default, this tests with mainnet resolver and modules unless RUN_OTHERS is set.
        bool runOthers = false;
        try vm.envBool("RUN_OTHERS") returns (bool val) {
            runOthers = val;
        } catch {}
        if (runOthers) {
            console.log("Running LiquidityBaseTest with non-mainnet (others) resolver and modules");
            userModule = new FluidLiquidityUserModuleOthers();
            adminModule = new FluidLiquidityAdminModuleOthers(1e10 * 1e18); // set native token max hard cap very high so it can be ignored for noramal tests
        } else {
            console.log("Running LiquidityBaseTest with mainnet resolver and modules");
            userModule = new FluidLiquidityUserModuleMainnet();
            adminModule = new FluidLiquidityAdminModuleMainnet(1e10 * 1e18); // set native token max hard cap very high so it can be ignored for noramal tests
        }
        ...
        adminSigs = [
            ...
            GuardianModule.pauseTokens.selector,
            GuardianModule.unpauseTokens.selector,
        ...
        // make sure test accounts are EOAs.
        vm.etch(alice, "");
        vm.etch(bob, "");
        vm.etch(admin, "");
