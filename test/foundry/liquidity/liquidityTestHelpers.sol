...
abstract contract TestHelpers is LiquiditySimulateStorageSlot, Test {
    ...
    function _resetDecay(address liquidity, address protocol, address token) internal {
        bytes32 slot = LiquiditySlotsLink.calculateDoubleMappingStorageSlot(
            LiquiditySlotsLink.LIQUIDITY_USER_SUPPLY_DOUBLE_MAPPING_SLOT,
            protocol,
            token
        );

        uint256 userSupplyData = IFluidLiquidity(liquidity).readFromStorage(slot);

        // Next  20 bits => 218-237 => decay percent (in 1e4: 1% = 10_000 -> max value 104_8575)
        // Next  10 bits => 238-247 => decay duration percent
        // mask to set decay bits to 0
        userSupplyData = (userSupplyData & 0xC000000003FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF);

        vm.store(liquidity, slot, bytes32(userSupplyData));
    }
    ...
    function _supply(
        ...
        // reset decay limit for all normal tests that are coded under the assumption decay limits dont exist. Decay is tested separately.
        _resetDecay(liquidity, address(mockProtocol), token);
    }

    function _supplyNative(address liquidity, MockProtocol mockProtocol, address user, uint256 amount) internal {
        vm.prank(user);
        ...
        // reset decay limit for all normal tests that are coded under the assumption decay limits dont exist. Decay is tested separately.
        _resetDecay(liquidity, address(mockProtocol), NATIVE_TOKEN_ADDRESS);
    }
    ...
    function _withdrawNative(address liquidity, MockProtocol mockProtocol, address user, uint256 amount) internal {
        vm.prank(user);
    ...
    function _supplyMax(MockProtocol protocol, address token, address user) internal {
        vm.startPrank(user);

        protocol.operate(token, int256(1e20), int256(0), address(0), address(0), abi.encode(user));
        protocol.operate(token, int256(1e24), int256(0), address(0), address(0), abi.encode(user));
        protocol.operate(token, int256(1e28), int256(0), address(0), address(0), abi.encode(user));
        protocol.operate(token, int256(1e32), int256(0), address(0), address(0), abi.encode(user));
        protocol.operate(token, int256(1e36), int256(0), address(0), address(0), abi.encode(user));

        // supply max
        protocol.operate(
            token,
            int256(type(int128).max) - (1e36 + 1e32 + 1e28 + 1e24 + 2e20),
            int256(0),
            address(0),
            address(0),
            abi.encode(user)
    ...
    function _borrowMax(MockProtocol protocol, address token, address user) internal {
        vm.startPrank(user);

        protocol.operate(token, int256(0), int256(1e20), address(0), user, abi.encode(user));
        protocol.operate(token, int256(0), int256(1e24), address(0), user, abi.encode(user));
        protocol.operate(token, int256(0), int256(1e28), address(0), user, abi.encode(user));
        protocol.operate(token, int256(0), int256(1e32), address(0), user, abi.encode(user));
        protocol.operate(token, int256(0), int256(1e36), address(0), user, abi.encode(user));

        protocol.operate(
            token,
            int256(0),
            int256(type(int128).max) - (1e36 + 1e32 + 1e28 + 1e24 + 3e20),
            address(0),
            user,
            abi.encode(user)
        );
