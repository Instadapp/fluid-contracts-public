...
import {
    ConstantVariables as FluidDexT1ConstantVariables
} from "../../../../contracts/protocols/dex/poolT1/common/constantVariables.sol";
...
contract PoolT1BaseTest is DexFactoryBaseTest {
    ...
    function setUp() public virtual override {
...

        {
            ...
            _setUpDexParams(
                DAI_USDC_WITH_80_20,
                1e27,
                1e4 * DAI_USDC_WITH_80_20.token0Wei,
                1e4 * DAI_USDC_WITH_80_20.token0Wei,
                ci_
            );
        }

        {
            ...
            _setUpDexParams(
                DAI_USDC_WITH_50_5,
                1e27,
                1e4 * DAI_USDC_WITH_50_5.token0Wei,
                1e4 * DAI_USDC_WITH_50_5.token0Wei,
                ci_
            );
        }

        {
            ...
            _setUpDexParams(
                DAI_USDC_WITH_10_1,
                1e27,
                1e4 * DAI_USDC_WITH_10_1.token0Wei,
                1e4 * DAI_USDC_WITH_10_1.token0Wei,
                ci_
            );
        }
    ...
    function _deployPoolT1(
    ...
    ) internal returns (DexParams memory dex) {
        ...
        vm.startPrank(alice);
        bytes memory poolT1CreationCode = abi.encodeCall(
            poolT1DeploymentLogic.dexT1,
            (dex.token0, dex.token1, oracleSlots_)
        );
    ...
    function _setUpDexParams(
        ...
        DexAdminStrcuts.InitializeVariables memory initialize_ = DexAdminStrcuts.InitializeVariables({
            ...
            thresholdShiftTime: initializeParams_.thresholdShiftTime == 0
                ? 1 days
                : initializeParams_.thresholdShiftTime,
...

        {
            ...
            uint256 ethValue_ = _getSmartDebtOrColEthAmount(
                dex,
                dex.dexColDebt,
                token0ColAmt + token0DebtAmt,
                centerPrice
            );
    ...
    function _getDexVariablesData(FluidDexT1 dex) internal view returns (DexVariablesData memory d_) {
        ...
        d_.lastToLastPrice =
            (d_.lastToLastPrice >> DEFAULT_EXPONENT_SIZE) <<
            (d_.lastToLastPrice & DEFAULT_EXPONENT_MASK);
    ...
    function _getDexVariables2Data(FluidDexT1 dex) internal view returns (DexVariables2Data memory d2_) {
        ...
        d2_.minCenterPrice =
            (d2_.minCenterPrice >> DEFAULT_EXPONENT_SIZE) <<
            (d2_.minCenterPrice & DEFAULT_EXPONENT_MASK);
        d2_.maxCenterPrice =
            (d2_.maxCenterPrice >> DEFAULT_EXPONENT_SIZE) <<
            (d2_.maxCenterPrice & DEFAULT_EXPONENT_MASK);
    }
    ...
    function _getDexSupplyData(FluidDexT1 dex, address user_) internal view returns (DexUserSupplyData memory us_) {
        ...
        us_.previousUserLimit =
            (us_.previousUserLimit >> DEFAULT_EXPONENT_SIZE) <<
            (us_.previousUserLimit & DEFAULT_EXPONENT_MASK);
    ...
    function _getDexBorrowData(FluidDexT1 dex, address user_) internal view returns (DexUserBorrowData memory ub_) {
        ...
        ub_.previousUserLimit =
            (ub_.previousUserLimit >> DEFAULT_EXPONENT_SIZE) <<
            (ub_.previousUserLimit & DEFAULT_EXPONENT_MASK);
    ...
    function _getOracleMapData(
        FluidDexT1 dex,
        uint256 oracleMap_,
        uint256 oracleSlot_
    ) internal returns (OracleMapData memory data_, OracleSlotConfig memory slotConfig) {
    ...
    function _getDexType(
        DexParams memory dexPool_,
        DexType dexType_,
        bool skipValidation
    ) internal returns (FluidDexT1 dex_) {
    ...
    function getState(DexParams memory dexPool_, FluidDexT1 dex_, address user_) internal returns (StateData memory) {
        ...
        (FluidLiquidityResolver.UserSupplyData memory userSupplyDataToken1_, ) = resolver.getUserSupplyData(
            address(dex_),
            dexPool_.token1
        );
        (FluidLiquidityResolver.UserBorrowData memory userBorrowDataToken0_, ) = resolver.getUserBorrowData(
            address(dex_),
            dexPool_.token0
        );
    ...
    function _getPricesAndExchangePrices(
        FluidDexT1 dex_
    ) internal returns (DexStrcuts.PricesAndExchangePrice memory pex_) {
        (bool status_, bytes memory returnData_) = address(dex_).call(
            abi.encodeWithSelector(FluidDexT1.getPricesAndExchangePrices.selector)
        );
    ...
    function _convertAdjustedToNormal(
        uint256 adjustedAmt_,
        uint256 weiAmount_
    ) internal returns (uint256 convertedAmount_) {
        if (weiAmount_ == 1e12) {
    ...
    function _testWithdrawPerfectInOne(
        ...
        (token0AmountWithdraw_, token1AmountWithdraw_) = withdrawInToken0
            ? (withdrawAmt_, uint256(0))
            : (uint256(0), withdrawAmt_);
    ...
    function _testPaybackPerfectInOneToken(
        ...
        (token0AmountPayback, token1AmountPayback) = paybackInToken0_
            ? (paybackAmt_, uint256(0))
            : (uint256(0), paybackAmt_);
    ...
    function _testDepositColLiquidityInWei(
        ...
        uint256 percentDiff = (share_ > JsOutput)
            ? ((share_ - JsOutput) * 10000) / share_
            : ((JsOutput - share_) * 10000) / JsOutput;
        assert(percentDiff <= 1);
        StateData memory postState_ = getState(dexPool_, dex_, address(user_));
        if (preState_.liquidityToken1SupplyReserve == 0 || preState_.liquidityToken0SupplyReserve == 0) {
            return share_;
    ...
    function _testSwapExactInBackAndForth(
        ...
        uint256 amountInBack_ = _testSwapExactIn(
            dexPool_,
            user_,
            amountOut_,
            !startSwap0To1,
    ...
    function _testUpdateRangePercents(
        ...
        assertEq(
            ...
            string(abi.encodePacked(assertMessage_, "upperPercent not match"))
        );
        assertEq(
            lowerPercent_,
            postDs_.d2.lowerPercent,
            string(abi.encodePacked(assertMessage_, "lowerPercent not match"))
        );
        ...
        // Dex Variables
        {
            ...
            assertEq(
                preDs_.d.centerPrice,
                postDs_.d.centerPrice,
                string(abi.encodePacked(assertMessage_, "centerPrice"))
            );
            assertEq(
        ...
        // Dex Variables 2
        {
            ...
            assertEq(
                preDs_.d2.revenueCut,
                postDs_.d2.revenueCut,
                string(abi.encodePacked(assertMessage_, "revenueCut"))
            );
    ...
    function _testUpdateCenterPriceAddress(
        ...
        if (centerPriceAddress_ > 0) {
            assertEq(
                block.timestamp,
                postDs_.cps.timestampOfShiftStart,
                string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData case 1: timestampOfShiftStart"))
            );
            assertEq(
                percent_,
                postDs_.cps.percentShift,
            ...
            assertEq(
                0,
                postDs_.cps.timestampOfShiftStart,
                string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData case 2: timestampOfShiftStart"))
            );
            assertEq(
                0,
                postDs_.cps.percentShift,
    ...
    function _testUpdateHookAddress(
        ...
        // Center Price shift
        {
            assertEq(
                preDs_.cps.timestampOfShiftStart,
                postDs_.cps.timestampOfShiftStart,
                string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData case 3: timestampOfShiftStart"))
            );
            assertEq(
                preDs_.cps.percentShift,
                postDs_.cps.percentShift,
    ...
    function _testUpdateUserSupplyConfig(
        ...
        string memory assertMessage_ = string(
            abi.encodePacked("_testUpdateUserSupplyConfig::", _getDexPoolName(dexPool_, dexType_), ":")
        );
    ...
    function _testUpdateUserBorrowConfig(
        ...
        string memory assertMessage_ = string(
            abi.encodePacked("_testUpdateUserBorrowConfig::", _getDexPoolName(dexPool_, dexType_), ":")
        );
    ...
    function _testSwapExactInForthEnablePoolSwapBack(
        ...
        for (uint256 i = 0; i < entries.length; i++) {
            ...
            if (dexType_ == DexType.SmartDebt) {
                (, int256 borrowAmount_, , , , ) = abi.decode(
                    l_.data,
                ...
                (int256 depositAmount_, , , , , ) = abi.decode(
                    l_.data,
                    (int256, int256, address, address, uint256, uint256)
                );
    ...
    function _testUpdateOracleWithTimes(
        ...
        vm.skip(true); // oracle logic removed
        FluidDexT1 dex_ = _getDexType(dexPool_, dexType_, false);
        ...
        for (uint256 i = 0; i < casesToRun_; i++) {
            ...
            OracleLoopData memory v_;
            {
                v_.changeInPriceBeforeSwap =
                    int256(ORACLE_PRECISION) -
                    int256((d_.lastToLastPrice * ORACLE_PRECISION) / d_.lastPrice);
                ...
                (v_.oracleMapData, v_.oracleSlotConfig) = _getOracleMapData(
                    dex_,
                    v_.dexVariablesBeforeSwap.oracleMap,
                    v_.dexVariablesBeforeSwap.oracleSlot
                );
    ...
    function _calculateRange(FluidDexT1 dex_) internal returns (RangeParams memory r_) {
        ...
        r_.upperThreshold = (r_.centerPrice +
            ((r_.upperRange - r_.centerPrice) * (1e3 - dexVariables2_.upperShiftPercent / 1e3)) /
            1e3);
        r_.lowerThreshold = (r_.centerPrice -
            ((r_.centerPrice - r_.lowerRange) * (1e3 - dexVariables2_.lowerShiftPercent / 1e3)) /
            1e3);
    }
    ...
    function _validateThresholdShift(
        ...
        v_.upperThreshold = (v_.centerPrice +
            ((v_.upperRange - v_.centerPrice) * (1e3 - v_.dexVariables2.upperShiftPercent / 1e3)) /
            1e3);
        v_.lowerThreshold = (v_.centerPrice -
            ((v_.centerPrice - v_.lowerRange) * (1e3 - v_.dexVariables2.lowerShiftPercent / 1e3)) /
            1e3);
        ...
        for (uint256 i = 0; i < v_.timeShifts.length; i++) {
            ...
            if (v_.dexVariables.lastPrice > v_.upperThreshold) {
                if (v_.totalShiftTime >= v_.shiftingTime) {
                    ...
                    changeCenterPrice_ =
                        v_.centerPrice +
                        (((v_.upperRange - v_.centerPrice) * v_.totalShiftTime) / v_.shiftingTime);
                }
            } else {
                if (v_.totalShiftTime >= v_.shiftingTime) {
        ...
        for (uint256 j = 0; j < p_.twaps.length; j++) {
...

            {
                assertApproxEqAbs(
                    ...
                    p_.aggTwap1By0,
                    1,
                    string(abi.encodePacked(assertMessage_, "twap_.twap1by0"))
                );
                assertApproxEqAbs(
                    twap0by1_,
                    p_.aggTwap0By1,
                    1,
                    string(abi.encodePacked(assertMessage_, "twap_.twap0by1"))
                );
            }
        }
    }
...
contract PoolT1Admin is PoolT1BaseTest {
    ...
    function testAdmin_UpdateHookAddress() public {
        vm.skip(true); // hook logic removed
        vm.prank(bob);
        contractDeployerFactory.deployContract(type(MockDexCenterPrice).creationCode);
        uint256[2] memory centerPriceAddressNonces_ = [contractDeployerFactory.totalContracts(), 0];
        DexType[3] memory poolTypes_ = [DexType.SmartColAndDebt, DexType.SmartCol, DexType.SmartDebt];
...

                    {
                        ...
                        _testWithdrawPerfectColLiquidity(
                            ...
                            abi.encodeWithSelector(
                                FluidDexErrors.FluidDexError.selector,
                                FluidDexTypes.DexT1__WithdrawLimitReached
                            )
                        );
                ...
                for (uint256 l = 0; l < 2; l++) {
...

                    {
                        ...
                        _testBorrowPerfectDebtLiquidity(
                            ...
                            abi.encodeWithSelector(
                                FluidDexErrors.FluidDexError.selector,
                                FluidDexTypes.DexT1__DebtLimitReached
                            )
                        );
                        vm.revertTo(snapshotId2_);
                    }
                    ...
                    {
                        // withdrawColLiquidity
                        (uint256 token0Amount_, uint256 token1Amount_) = _testBorrowPerfectDebtLiquidity(
                            pools[i],
                            eve,
                            50 * 1e18,
                            poolTypes_[j],
    ...
    function testDexT1_WithdrawPerfectColLiquidity() public {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                ...
                (uint256 token0AmountDeposit_, uint256 token1AmountDeposit_) = _testDepositPerfectColLiquidity(
                    dexPool_,
                    alice,
                    sharesAmount_,
                    poolTypes_[j]
                ...
                (uint256 token0AmountWithdraw_, uint256 token1AmountWithdraw_) = _testWithdrawPerfectColLiquidity(
                    dexPool_,
                    alice,
                    sharesAmount_,
                    poolTypes_[j],
    ...
    function testDexT1_PaybackPerfectDebtLiquidity() public {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                ...
                (uint256 token0AmountBorrow_, uint256 token1AmountBorrow_) = _testBorrowPerfectDebtLiquidity(
                    pools[i],
                    alice,
                    sharesAmount_,
                    poolTypes_[j],
                ...
                (uint256 token0AmountPayback, uint256 token1AmountPayback) = _testPaybackPerfectDebtLiquidity(
                    pools[i],
                    alice,
                    sharesAmount_,
                    poolTypes_[j]
    ...
    function testDexT1_DepositColLiquidityInTwoHalfs() public {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                ...
                (uint256 token0Amount_, uint256 token1Amount_) = _testDepositPerfectColLiquidity(
                    pools[i],
                    alice,
                    shareAmount_,
                    poolTypes_[j]
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                for (uint256 k = 0; k < a_.length; k++) {
                    ...
                    uint256 depositShares_ = _testDepositColLiquidity(
                        pools[i],
                        alice,
                        a_[k].token0Amount,
                        a_[k].token1Amount,
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                for (uint256 k = 0; k < a_.length; k++) {
                    ...
                    uint256 withdrawShares0_ = _testWithdrawColLiquidity(
                        pools[i],
                        alice,
                        a_[k].token0Amount,
                        0,
                    ...
                    uint256 withdrawShares1_ = _testWithdrawColLiquidity(
                        pools[i],
                        alice,
                        0,
                        a_[k].token1Amount,
            ...
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                for (uint256 k = 0; k < a_.length; k++) {
                    ...
                    );
                    {
                        ...
                        (uint256 tokenDeposit0_, uint256 tokenDeposit1_) = _testDepositPerfectColLiquidity(
                            pools[i],
                            alice,
                            a_[k].shares,
                            poolTypes_[j]
            ...
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                for (uint256 k = 0; k < a_.length; k++) {
                    ...
                    );
                    {
                        ...
                        (uint256 tokenBorrow0_, uint256 tokenBorrow1_) = _testBorrowPerfectDebtLiquidity(
                            pools[i],
                            alice,
                            a_[k].shares,
                            poolTypes_[j],
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                for (uint256 k = 0; k < 2; k++) {
                    ...
                    uint256 share_ = _testDepositColLiquidity(
                        pools[i],
                        alice,
                        a_[k].token0Amount,
                        a_[k].token1Amount,
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                for (uint256 k = 0; k < 2; k++) {
                    ...
                    (uint256 token0AmountPayback_, uint256 token1AmountPayback_) = _testPaybackPerfectInOneToken(
                        pools[i],
                        alice,
                        1e3 * 1e18,
                        k == 0,
...
contract PoolT1Oracle is PoolT1BaseTest {
    ...
    function testDexT1_OraclePriceCaseOne() public {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            for (uint256 j = 0; j < poolTypes_.length; j++) {
                string memory assertMessage_ = string(
                    abi.encodePacked("_testOraclePrice::", _getDexPoolName(pools[i], poolTypes_[j]), ":", " ")
                );
