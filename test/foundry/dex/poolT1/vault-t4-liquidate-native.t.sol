...
contract VaultT4LiquidateNativeWithAsborb is Test {
    ...
    function test_liquidateWithAbsorbNative() public {
        ...
        FluidVaultResolver.VaultEntireData memory vaultData_ = vaultResolver.getVaultEntireData(
        ...
        vaultData_ = vaultResolver.getVaultEntireData(address(VAULTT4_WSTETH_ETH));
        console2.log("new Oracle Price is:", vaultData_.configs.oraclePriceLiquidate);

        // check liquidation available
        FluidVaultResolver.LiquidationStruct memory liquidationData = vaultResolver.getVaultLiquidation(
            address(VAULTT4_WSTETH_ETH),
            1e40
        );
