        ...
        if (vaultType_ == VaultType.VaultT1) {
            ...
            if (params_.debtToken0Percentage > 0 && params_.debtToken1Percentage > 0) {
                ...
                if (vaultType_ == VaultType.VaultT3) {
                    ...
                    vm.prank(user_);
                    (d_.debtShares, d_.token0Debt, d_.token1Debt, d_.colShares, d_.token0Col, d_.token1Col) = dexVault_.vaultT4.liquidatePerfect(
                       l_.debtAmount * params_.debtToken0Percentage / 100,
                        l_.debtToken0Amt, 
                        l_.debtToken1Amt,
                        0, // colPerUnitDebt
                ...
                if (vaultType_ == VaultType.VaultT3) {
                    vm.prank(user_);
                    (d_.debtShares, d_.colShares) = dexVault_.vaultT3.liquidate(
                        d_.token0Debt,
                        d_.token1Debt,
                        1, // debtShareMin
                        0, // colPerUnitDebt
       ...
       OperateParams[4] memory params_ = [
        ...
        for(uint256 i = 0; i < vaults_.length; i++) {
            for(uint256 j = 0; j < vaultTypes_.length; j++) {
                ...
                _setOraclePrice(vaults_[i], vaultTypes_[j], _getOraclePrice(vaults_[i], vaultTypes_[j]) * 900 / 1000); // decrease by 10%
