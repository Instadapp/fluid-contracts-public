...
contract VaultT1Test is VaultT1BaseTest {
    ...
    function testWithdrawAndPaybackMax() public {
        ...
        uint usdcBalLiq_ = IERC20(address(USDC)).balanceOf(address(liquidity));
        uint daiBalLiq_ = IERC20(address(DAI)).balanceOf(address(liquidity));
        ...
        // Use approxEq with max relative diff of 0.00001% (1e-7)
        assertApproxEqRel(
            usdcBalLiq_ - IERC20(address(USDC)).balanceOf(address(liquidity)),
            uint256(supply_),
            1e11, // 0.00001% max diff
            "USDC liquidity balance not as expected after withdrawMax"
        );
        assertApproxEqRel(
            IERC20(address(DAI)).balanceOf(address(liquidity)) - daiBalLiq_,
            uint256(borrow_),
            1e11, // 0.00001% max diff
            "DAI liquidity balance not as expected after paybackMax"
        );
    }
