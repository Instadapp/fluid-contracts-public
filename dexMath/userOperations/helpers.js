...
function _calculateReservesOutsideRange(geometricMeanPrice, priceAtRange, reserveX, reserveY) {
    ...
    const part2 = (
        (geometricMeanPrice * reserveX + reserveY * SCALE) / 
        (2 * part1)
    );
...
function _withdrawAdjusted(token0AmtAdjusted, token1AmtAdjusted, slippage, dexFee, totalSupplyShares, colReserves, pex) {
    let temp = 0;
    let temp2 = 0;
    let shares = 0;
    let sharesWithSlippage = 0;
    ...
    if (pex.geometricMean < 1e27) {
        const ob_ = _calculateReservesOutsideRange(pex.geometricMean, pex.upperRange, colReserves.token0RealReserves - temp, colReserves.token1RealReserves - temp2);
        token0ImaginaryReservesOutsideRange = ob_.reserveXOutside;
        token1ImaginaryReservesOutsideRange = ob_.reserveYOutside;
    } else {
        const ob_ = _calculateReservesOutsideRange(1e54 / pex.geometricMean, 1e54 / pex.lowerRange, colReserves.token1RealReserves - temp2, colReserves.token0RealReserves - temp);
 ...
 * @param {number|bigint} geometricMean - Geometric mean of upper and lower price ranges (in 1e27 decimals)
 * @param {number|bigint} lowerPrice - Lower price range (in 1e27 decimals)
 * @param {number|bigint} debtA - Debt amount of token A
 * @param {number|bigint} debtB - Debt amount of token B
...
function _calculateDebtReserves(geometricMean, lowerPrice, debtA, debtB) {
    ...
    const part1 = (debtA * geometricMean - debtB * E27) / (2 * E27);
    
    // part2 = (debtA * debtB * lowerPrice) / 1e27
    let part2 = debtA * debtB;
    part2 = part2 < E50 ? (part2 * lowerPrice) / E27 : (part2 / E27) * lowerPrice;
    
    // realDebtReserveB = part1 + sqrt(part2 + part1^2)
    const realDebtReserveB = part1 + Math.sqrt(part2 + part1 * part1);
...
function _borrowAdjusted(token0AmtAdjusted, token1AmtAdjusted, slippage, dexFee, totalBorrowShares, debtReserves, pex) {
    ...
    if (pex.geometricMean < 1e27) {
        const ob_ = _calculateDebtReserves(pex.geometricMean, pex.lowerRange, debtReserves.token0Debt + temp, debtReserves.token1Debt + temp2);
        token0FinalImaginaryReserves = ob_.imaginaryDebtReserveA;
        token1FinalImaginaryReserves = ob_.imaginaryDebtReserveB;
    } else {
        const ob_ = _calculateDebtReserves(1e54 / pex.geometricMean, 1e54 / pex.upperRange, debtReserves.token1Debt + temp2, debtReserves.token0Debt + temp);
