...
function swapInAdjusted(
    ...
    if (amountInCollateral > 0) {
        let reservesRatioValid = swap0To1
          ? helpers.verifyToken1Reserves(colReserveIn + amountInCollateral, colReserveOut - amountOutCollateral, oldPrice)
          : helpers.verifyToken0Reserves(colReserveOut - amountOutCollateral, colReserveIn + amountInCollateral, oldPrice);
    ...
    if (amountInDebt > 0) {
        let reservesRatioValid = swap0To1
            ? helpers.verifyToken1Reserves(debtReserveIn + amountInDebt, debtReserveOut - amountOutDebt, oldPrice)
            : helpers.verifyToken0Reserves(debtReserveOut - amountOutDebt, debtReserveIn + amountInDebt, oldPrice);
        if (!reservesRatioValid) {
            return 0;
        }
    }
